...
这份文件是**产品决策的真相源**。审计阶段的 OPEN_QUESTIONS.md 里那 13 个问题全部有了答案；其它文件里凡与本文件冲突的描述，都以本文件为准，并已就地改掉。
...
| ✅ | Stage B1（应用外壳 + 设计系统定值）与 Stage B2（共享基元）—— 见 STAGE_B_REPORT.md |
...
**已同步改掉**：UI_ARCHITECTURE_PROPOSAL.md §4.2/§4.4/§4.5 里的选择列、批量条与 `rowSelection` 映射。
...
**已同步改掉**：UI_ARCHITECTURE_PROPOSAL.md §5.6 的「待定」。
...
**已同步改掉**：UI_ARCHITECTURE_PROPOSAL.md §4.2 的列定义、[UI_AUDIT.md §6](UI_AUDIT.md) 的专项表。

### 2.4 第一版没有批量，所以删掉相关设计

见 D3。已从 UI_ARCHITECTURE_PROPOSAL.md 删除首版的 checkbox 列、`rowSelection` 与批量操作条。
...
| 文件 | 改了什么 |
|---|---|
| OPEN_QUESTIONS.md | 13 问全部标记为已决，指向本文件；保留延期项索引 |
| UI_ARCHITECTURE_PROPOSAL.md | 队列四页签与 `queue` 参数；详情 URL 上下文；删译文四态列；删首版批量；标签 chips；AntD 三项核验原则；历史分页 |
...
| SCREEN_INVENTORY.md | 无需改（它只描述现状） |
