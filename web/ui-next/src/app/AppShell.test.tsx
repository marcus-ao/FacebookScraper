...
describe('顶栏', () => {
  ...
  it('运行状态入口是中性的，不编造「运行正常」', () => {
    const html = render('/review')
    // Stage B1 不做真实状态映射，所以不许出现绿点或"正常"。
...
describe('内部核验页', () => {
  it('/_internal/design-check 顶上挂了"这不是业务界面"的说明', () => {
    const html = render('/_internal/design-check')
    expect(html).toContain('内部核验页')
    expect(html).toContain('生产导航里没有入口')
  })

  it('主导航里没有它的入口', () => {
    const html = render('/review')
    expect(html).not.toContain('design-check')
  })
})
