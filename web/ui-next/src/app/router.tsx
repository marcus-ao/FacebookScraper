 ...
 * 旧 URL 的入口。
 *
 * ⚠️ 永久保留（DECISION_LOG.md D11）：web/DESIGN.md §11 规定飞书卡片里带
...
/**
 * 路由表。详情用两条父路径挂同一个界面，这样"从哪来回哪去"由路由结构表达，
 * 不再靠旧 App.vue:57 那个 `if (view !== 'history')` 判断。
 *
...
export const routes: RouteObject[] = [
  {
    ...
    children: [
      ...
      {
        // DECISION_LOG.md D9：路由继续存在，但**不在主导航里**。
      ...
      {
        // 共享基元的核验页。**生产导航里没有入口**，`internal: true` 会让外壳
        // 在页面顶部挂一条说明，直达时也一眼看得出不是业务界面（本会话 §28）。
        path: '_internal/design-check',
        element: <DesignCheck />,
        handle: meta({ title: '设计核验', internal: true }),
      },
