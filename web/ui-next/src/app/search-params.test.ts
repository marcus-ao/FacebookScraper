...
// 这组用例守的是 DECISION_LOG §2.1–2.2 的两条架构修正。
// 它们在 Stage A 就能测，因为 URL 契约是纯函数——不需要界面。
...
describe('列表查询参数解析（详情页要用同一份来算 n / N）', () => {
  ...
  it('历史：不在白名单里的 limit 回落 50 —— 旧 UI 硬传的 30 也归这里', () => {
...
describe('旧 URL 永久兼容（DECISION_LOG D11）', () => {
  ...
  it('未知 view 回落审校队列 —— 与旧 App.vue:20 的白名单行为一致', () => {
