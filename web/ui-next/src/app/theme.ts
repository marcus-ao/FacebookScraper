 ...
 * 设计 token 的**唯一真相源**。
 *
 * DESIGN.md §2 的规矩：颜色、间距、字号、圆角、高度、阴影参数的字面值
 ...
 *
 * DECISION_LOG.md §5.2：如果 antd 的 `theme.cssVar` 行为不符合预期，
 * **不许 hack** —— 就用这里的同一个对象生成各路，仍然只有一个真相源。
 * 本文件走的就是这条路：不依赖 cssVar 也成立，开了 cssVar 只是多一层便利。
 */

/** 中性阶。沿用旧 UI 的 slate 系（已去色相偏移），DESIGN.md §5.3。 */
const neutral = {
  bg: '#f1f5f9',
  surface: '#ffffff',
...
/**
 * 状态色。红与黄的具体值沿用旧 UI —— 它们已经在真实的中英德混排
 * （大量 emoji）正文上被看过，换值没有收益（DESIGN.md §5.4）。
 *
 * ⚠️ 内容区里只有这两种饱和色：红=这里错了，黄=这里注意看。
 * 混成一种，用几次她两种都不信。
 *
 * ⛔ 状态色不表达动作（DESIGN.md §5.1 第 4 条）：
...
/**
 * ── 主色（Stage B1 定值，这是最终值）────────────────────────────────────────
 *
 * **#155EEF。** 对白色 **5.41:1**（算法见 src/lib/contrast.ts，断言见 theme.test.ts）。
 *
 * 为什么不是 Stage A 占位的 `#1677ff`：它对白色只有 **4.10:1**，达不到
 * DESIGN.md §5.2 要求的 4.5:1。那条不是审美偏好 —— 主按钮上的白字是
 * 「通过并创建排期」这种不能看错的文案，而她一天要按几十次。
 *
 * 为什么不是旧 UI 的 `#4f46e5`：色相 ~244°，正好落在提纲点名要避开的
 * "通用 AI 紫"区间。#155EEF 的色相 ~220°，是明确的冷蓝。
 *
 * 逐条对 DESIGN.md §5.2 的判据（全部在 theme.test.ts 里算，不靠肉眼）：
 *
...
/** 8px 基准，六档，刻度外的值不许出现（DESIGN.md §4）。 */
const space = { s1: 4, s2: 8, s3: 12, s4: 16, s5: 24, s6: 32 } as const

/** 七级排版（DESIGN.md §3.1）。正文对照的 15/1.7 是唯一例外，见 §3.2。 */
const typography = {
  pageTitle: 20,
  sectionTitle: 14,
  subsectionTitle: 13,
...
/** 固定高度（DESIGN.md §4）。48px 行高的推导见 §4.1。 */
const layout = {
  topbarHeight: 48,
  sidebarWidth: 200,
  ...
  navItemHeight: 40,
  /** 列表行里的缩略图。48px 行高里上下各留 4px（DESIGN.md §4.1）。 */
  thumbnailSize: 40,
  iconSize: 16,
  borderRadius: 6,
  /** 焦点环：2px 主色 + 2px offset，全应用一种（DESIGN.md §13）。 */
  focusRingWidth: 2,
  focusRingOffset: 2,
...
/**
 * 阴影。**只有浮层有阴影**（DESIGN.md §6.3）。
 *
 * 旧 UI 给每个列表行都加了两层阴影，17 行就是 34 个阴影在一屏里。
 * 这里只留 antd 浮层要用的那一组，页面内的表面一律靠 1px 边框分层。
 */
const elevation = {
  overlay: '0 6px 16px rgba(15, 23, 42, 0.12), 0 3px 6px rgba(15, 23, 42, 0.08)',
} as const

/** 动效 2/10：只有状态过渡，没有入场编排（DESIGN.md §12）。 */
const motion = {
  duration: '0.18s',
...
export const antdToken = {
  ...
  // DESIGN.md §5.4：info 就是主色，不另开一种蓝。
  ...
  // 拉回我们的七级刻度，否则 Stage C/D 随手写一个 <Title level={2}> 就是 30px，
  // 「同级必须同字号」当场破掉（旧 UI 的 h3 有 12/13/14 三种，就是这么来的）。
...
export const cssVariables: Readonly<Record<string, string>> = {
  ...
  /** 吸顶补偿用它，不再像旧 UI 那样写死 96px（UI_AUDIT P2-2）。 */
