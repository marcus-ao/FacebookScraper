...
/**
 * 「这一篇有没有事」—— 与「处理到哪一步了」分开的那一列（DESIGN.md §7.1）。
 *
 * 状态回答进度，问题指示回答风险。两者是不同的列，不要混。
 *
 * **列表里的红色只出现在这一列。** 位置固定，她扫一列就知道哪几行要停下来。
 * 旧 UI 的反面：一屏 17 个红色「这篇不发」按钮 + 0 个主按钮，
 * 于是红色不再是信号（UI_AUDIT P0-2）。
 *
 ...
 * 把详情字段当列表字段用是 DECISION_LOG §5.4 明令禁止的三条之一。
 */
...
export function ProblemIndicator({ source }: ProblemIndicatorProps) {
  ...
  if (kind === 'none') {
    // 没事就什么都不画。"一切正常"不需要一个图标来宣告（DESIGN.md §10.3）。
