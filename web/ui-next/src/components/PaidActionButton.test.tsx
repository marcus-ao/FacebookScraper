...
describe('金额写在按钮上', () => {
  it('动作 + 金额同时可见', () => {
    // 旧 UI 的「生成图片 · 约 US$0.045」是对的，保留这个形状。
    const markup = html(<PaidActionButton label="图片优化" amount="US$0.045" />)
...
describe('disabled 必须给原因', () => {
  ...
  it('旧 UI 那种"六个条件的或 + 一个灰按钮"在这里不可能出现', () => {
    // ApprovalPanel.disabled = editing || loading || submitting || !eligible
    //                          || !options?.available || !when
    // 界面上只有一个灰按钮，她无从知道要先做什么（UI_AUDIT P2-10）。
    // 这里调用方必须先把那六个条件收敛成一句话才能让按钮变灰。
    const markup = html(
      <PaidActionButton
        label="文案优化"
        amount="US$0.012"
