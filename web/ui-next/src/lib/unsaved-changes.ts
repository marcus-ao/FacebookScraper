 ...
 *
 * 旧 UI 是三层手写（BASELINE_BEHAVIOR.md §2.3）：
 *
 *   应用内导航    App.mayLeave() 串联子组件的 defineExpose
 *   浏览器前进后退 restoreLocation() 被拒后 writeLocation() 把 URL 推回去
 *   关闭标签页    TaskDetail 与 SettingsPanel 各自的 beforeunload
 *
 * 三层都必须保住。守卫本身抽到一处的理由：**详情编辑与设置编辑要用同一套**，
 * 各写一份就会像旧 UI 那样出现两份判据、两句文案、两种边界情况。
 */
 ...
 * 出处：web/ui/src/App.vue 与 SettingsPanel.vue 的 window.confirm。
 */
