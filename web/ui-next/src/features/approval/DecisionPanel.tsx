import { Alert, Button, Collapse, Input, Modal, Space, Tooltip, Typography } from 'antd'
...
import { berlinInput, wallMinutesApart, AUTHOR_KIND_LABEL, formatDate } from '@/lib/format'
import { isConflict } from '@/services/http'
import styles from './DecisionPanel.module.css'

export function ApprovalAction({ controller: c }: { controller: ApprovalController }) {
  return <Tooltip title={c.reason}><span title={c.reason}><Button aria-label="通过并创建排期" type="primary" disabled={!!c.reason} loading={c.busy} onClick={c.open}>通过并创建排期</Button></span></Tooltip>
}
...
export function DecisionPanel({ detail, controller: c, editing }: { detail: TaskDetail; controller: ApprovalController; editing: boolean }) {
  ...
  const occupied = calendar.data?.cards.filter(card => card.channels.includes(detail.platform) && card.at_business.slice(0, 10) === c.when.slice(0, 10)) ?? []
  const near = occupied.some(card => wallMinutesApart(c.when, card.at_business) < (calendar.data?.gap_minutes ?? 0))
  return <section className={styles.panel} aria-label="审核与排期">
    ...
    {min && max && <div className={styles.help}>可选 {min.replace('T', ' ')} 至 {max.replace('T', ' ')}（柏林） <Space wrap>{c.eligible && data?.default_times.map(time => <Button size="small" key={time} disabled={editing || c.busy || !data.available} onClick={() => c.setWhen(c.when.slice(0, 10) + 'T' + time)}>{time} 柏林</Button>)}</Space></div>}
    {c.eligible && <Collapse ghost items={[{ key: 'occupancy', label: `${near ? '附近已有同渠道排期 · ' : ''}查看当天同渠道占用（间隔 ${calendar.data?.gap_minutes ?? '—'} 分钟）`, children: <>
      ...
      {occupied.length ? occupied.map((card, i) => <p key={i}><BerlinTime at={card.at_business} /> · {card.delivery === 'published' ? '已观测到公开发布' : card.delivery === 'scheduled' ? '已创建定时任务' : '发布状态待核验'}</p>) : <p>当前缓存未发现这一天的同渠道记录，不能据此保证空闲。</p>}
    </> }]} />}
