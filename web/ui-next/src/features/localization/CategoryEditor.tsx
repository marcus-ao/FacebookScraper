...
export function CategoryEditor({ detail, disabled, apply, refresh }: { detail: TaskDetail; disabled: boolean; apply: (detail: TaskDetail) => void; refresh: () => Promise<TaskDetail> }) {
  ...
  return <Space wrap><span>产品分类</span>{detail.tags.length ? detail.tags.map(tag => <Tag key={tag}>{tag}</Tag>) : <span>未分类</span>}{!detail.read_only && <Button type="text" size="small" disabled={disabled} onClick={() => { setInput(detail.tags.join('，')); setError(null); setOpen(true) }}>编辑分类</Button>}
    <Modal title="编辑产品分类" open={open} onCancel={() => { if (!busy) setOpen(false) }} okText="保存分类" cancelText="取消" confirmLoading={busy} onOk={() => void save()}>
      <label>多个分类用逗号隔开；清空后归为未分类。<Input.TextArea aria-label="产品分类" value={input} onChange={event => setInput(event.target.value)} disabled={busy} rows={2} /></label>
      {error ? isConflict(error) ? <ConflictRecovery kind="tags" recovering={busy} onRecover={() => { setBusy(true); void refresh().then(() => setError(null)).catch(setError).finally(() => setBusy(false)) }} /> : <Alert type="error" title="分类未保存，请重试" /> : null}
    </Modal>
  </Space>
}
