...
> **位置说明**：这份文档原本在 `docs/ui-refactor/` 下，与重构期的阶段报告和取证
> 产物放在一起。切换到 React 时那批一次性材料没有并入主干；这一份因为被
> `web/ui-next/src/**` 的代码注释直接引用，所以搬到了代码旁边。
> 文中指向阶段报告、审计快照等已移除文件的链接不再可达，内容可在
> `codex/business-workflow` 分支的 `227ddba` 提交里找到。

**成文日期：2026-09-13。** 前置：[BASELINE_BEHAVIOR.md](BASELINE_BEHAVIOR.md)（要保住的行为）、UI_ARCHITECTURE_PROPOSAL.md（要做成的形状）。

**执行状态：Stage A–J 已完成，生产 cutover 未执行。** 下文保留原迁移设计；当前执行清单与实际文件映射见 IMPLEMENTATION_PLAN.md，最终结果、B1–B24、切换及回滚步骤见 FINAL_IMPLEMENTATION_REPORT.md。原有阶段批准停点已由 2026-09-13 的连续实施授权替代。
...
这些是写类型时暴露出来的，**我没有替它们下结论**，逐条也在 OPEN_QUESTIONS.md 里：
...
按 UI_ARCHITECTURE_PROPOSAL.md §13 的四类划分，**只有第一类进 Query**。具体不进的：编辑草稿、标记游标、图片 `seen` 集合、当前图片索引、模态框开关、当前标签页、侧边导航折叠。
...
**从仓库现状推导，不照抄通用模板。** 依据两条：后端按路由模块分文件（§5.1），前端界面按 UI_ARCHITECTURE_PROPOSAL.md §3 分六个。
...
评估与调整的理由在 UI_ARCHITECTURE_PROPOSAL.md §15。
...
C 与 E′ 相邻是刻意的：两者共用 `PostRow`，隔着 D 做完再回来会让共享层被详情的需求带偏（UI_ARCHITECTURE_PROPOSAL.md §15）。
