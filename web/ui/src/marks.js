...
//
// 两件事必须分开（PROTOTYPE_DESIGN.md 第 8 节）：
...
const PRIORITY = { [MARK_ERROR]: 3, [MARK_RISK]: 2, [MARK_WARN]: 1 }
 ...
 */
export function buildMarks(highlights, risks) {
  const marks = []
...
export const charLength = (text) => codepoints(text).length
 ...
 */
export function segment(text, marks, side) {
  const value = text || ''
  const chars = codepoints(value)
  const cut = (from, to) => chars.slice(from, to).join('')
  const spans = []
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    out.push({
      ...
      index: covering.reduce(
        (min, s) => Math.min(min, s.mark.index), Number.MAX_SAFE_INTEGER)
    })
  }
