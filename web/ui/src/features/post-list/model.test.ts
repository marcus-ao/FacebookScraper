...
describe('review query 与行补丁', () => {
  ...
  it('计数使用 summary，不取当前 visible rows', () => {
    const summary = { ...data.summary, by_status:{ pending_review:12, edited:3, content_locked:2, not_ready:7, snoozed:4, approved:1, scheduled:2, skipped:3, handed_off:4 } }
    // 冻结的帖子仍算待审：人还要给它选时刻。
    expect(queueCounts(summary)).toEqual({review:17,not_ready:7,snoozed:4,processed:10})
  })
  it('详情 mutation 就地更新行和计数，人工优先且不修改原缓存', () => {
    const previous = { ...data.tasks[0]!, id: detail.id, status:'pending_review' as const }
    const input = { ...data, tasks:[previous], summary:{...data.summary,by_status:{not_ready:0,edited:0,content_locked:0,snoozed:0,approved:0,scheduled:0,skipped:0,handed_off:0,pending_review:1}} }
