...
/**
 * 帖子行的列工厂。**审校队列与历史归档共用这一套。**
 *
 * 共享列定义使状态、平台和缩略图在两个列表中保持一致。
 *
 * ──────────────────────────────────────────────────────────────────────────
 * 三条硬约束
 * ──────────────────────────────────────────────────────────────────────────
 ...
 * **1. 没有批量，所以没有勾选。**
 *
 * DECISION_LOG：第一版不显示 checkbox 列、不启用 `rowSelection`、
 ...
 *
 * ⛔ 所以**没有「译文来源」四态列**（DECISION_LOG）。列表 API 无法区分
...
/** 表格的固定行高。48px 的推导见 DESIGN.md（1366×768 首屏 12 行）。 */
export const POST_ROW_HEIGHT = layout.tableRowHeight

/** 最多两个分类，多的收进 `+N`。一屏 Tag 不超过 8 个（DESIGN.md）。 */
