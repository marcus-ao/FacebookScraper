...
export function MonitorPanel({ data, busy, action }: { data: MonitorStatus | undefined; busy: boolean; action: (operation: () => Promise<unknown>) => Promise<void> }) {
  ...
  return <section aria-label="新帖采集状态"><h2>新帖采集状态</h2>
    ...
    <p>每天 08:00–19:00 间隔 45–75 分钟，离岗间隔 135–225 分钟；06:30–07:30 一次受限补扫。</p>
    ...
    <Table<CaptureItem> rowKey="key" size="small" dataSource={[...items].reverse()} scroll={{ x: 820 }}
      ...
      { title: '发现 / 结束', render: (_, item) => <><ShanghaiTime at={item.first_seen_at} /><br /><ShanghaiTime at={item.finished_at} /><br />发现等待 {item.discovery_wait_seconds == null ? '未知' : `${Math.round(item.discovery_wait_seconds / 60)} 分钟`} · 本次抓取 {item.capture_seconds == null ? '未知' : `${Math.round(item.capture_seconds)} 秒`}</> },
