...
function RuntimeView({ initial, current }: { initial: RuntimeSnapshot; current: RuntimeSnapshot }) {
  ...
  const mirror = data.stages.find(stage => stage.number === 2)?.mirror
  const mirrorNeedsAttention = mirror && ['uncertain', 'blocked'].includes(mirror.status)
...
function MirrorFact({ mirror }: { mirror: NonNullable<RuntimeSnapshot['stages'][number]['mirror']> }) {
  const summary = mirrorSummary(mirror)
  return <p className={styles.help}>云盘镜像：{summary.label}。{summary.conclusion}
    {mirror.last_success_at && <> 最近成功 <ShanghaiTime at={mirror.last_success_at} />。</>}
    {mirror.last_error && <> {mirror.last_error}</>}
  </p>
}
