...
export function CalendarPage() {
  ...
  return <section aria-label="发布月历" aria-busy={busy}>
    ...
    <p className={styles.help}>月份按发布后台的 {data?.ui_timezone ?? '已配置时区'} 计算。已核实渠道和时间的内容遵循同渠道 {data?.gap_minutes ?? 90} 分钟间隔。明细未读取、渠道未知或时刻未独立核实的条目会阻止空档确认。选时提示来自这份缓存，正式提交前会再读后台；目标排期回读核实相关卡片，远端删除核实仍要求整月决策完整。</p>
