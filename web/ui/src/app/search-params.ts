 ...
 *
 * 这个文件实现 DECISION_LOG.md 的两条架构修正：
 *
...
/** 四个分桶各自包含哪些真实 status（DECISION_LOG.md）。 */
...
/** 历史分页：默认 50，可选 20 / 50 / 100（DECISION_LOG.md）。 */
 ...
 *
 * 这是「返回列表恢复原来的筛选和页码」的实现（DECISION_LOG.md）。
 */
...
// ─── 旧 URL 重定向（DECISION_LOG.md：永久保留，不设期限） ────────────────

/**
 * 兼容既有链接：`/?task=<account>/<post_id>` 与 `/?view=<name>`。
 *
 * ⚠️ 不能废：web/DESIGN.md 规定飞书卡片里带审校链接，外部系统里可能
 * 已经散落着这种 URL。
 *
