import type { ContentJob, InitialTranslationCapabilities, PromptTemplate, RefinementCapabilities, TaskDetail } from '@/types/domain'
...
export const refine = (detail: TaskDetail, kind: 'text' | 'image', instruction: string, mediaIndex: number) => request<ContentJob>(`/api/refinements/task/${idPath(detail.id)}`, jsonBody({ kind, instruction, media_index: kind === 'image' ? mediaIndex : null, ...jobVersions(detail) }))
