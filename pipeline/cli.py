...
def main(argv=None) -> int:
    ...
    notify_parser.add_argument('--dry-run', action='store_true',
                               help='配合 --self-test 仅输出四类示例卡片 JSON，不读凭据或发送消息')
    ...
    if args.command == 'notifications':
        from core.feishu import BOT_LABELS, FeishuSettings, Outbox, WebhookBot
        from core.feishu_cards import selftest_cards
        try:
            ...
            if args.dry_run and not args.self_test:
                raise ValueError('--dry-run 须与 --self-test 一起使用')
            settings = FeishuSettings.load()
            if args.self_test:
                cards = selftest_cards(settings)
                if args.dry_run:
                ...
                try:
                    ...
                    for example in cards:
                        role, card = example['role'], example['card']
