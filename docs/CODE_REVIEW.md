...
### 18.6 追加（同日）· CR-65 · 光"装得上"不够，还得"掉了会自己回来"

**用户复跑后仍然丢**：新开一个空标签页 → 点 shortcut 跳到
`business.facebook.com` → 之后全部没记录（只记下了 NTP 上那一次点击）。

#### 复现尝试：scratch 环境**怎么都复现不出来**

在一次性 Chrome 上把能想到的路径全试了一遍，**每一条都正常**：

| 场景 | 结果 |
|---|---|
| `chrome://new-tab-page` → 同标签页导航到本地站点 → 点击 | ✅ 记录 |
| 新开标签页（`/json/new`，Playwright 之外）→ 导航 → 点击 | ✅ 记录 |
| 跨站导航到真实 HTTPS（换渲染进程）→ 点击 | ✅ 记录 |
| 安装完**立刻**跨站导航 → 点击 | ✅ 记录 |
| **在用户自己的浏览器里**开中立标签页 → 安装 → 可信点击 | ✅ 记录 |

> ⚠️ 中途有一次误判：跨站后点击"没记录"，查下来是我点在了空白处，
> `event.target` 是 `<body>`，而 `_safe_element` **本来就故意丢弃 body/html**。
> 差一点把一个正确的过滤当成 bug 去"修"。**先证明点到了东西，再谈丢事件。**

#### 结论与处置：不再猜原因，改成**自愈**

复现不出来，就不该继续猜是哪一种原因（跨进程导航？装到一半页面跳走？
Playwright 的 page 对象本身是坏的？）。**换成一个对所有原因都成立的做法**：

1. **每 2 秒回读一次监听器标记，掉了就地补装**（`repair_if_lost`）。
   不管因为什么丢的，最多两秒自己回来。
2. **主帧一导航就立刻补注入一次**（`Page.frameNavigated`）——
   给"立刻"，巡检给"最多两秒"，两层都便宜，都留着。
3. **已装表改用 page 对象本身做键**，不用 `id(page)`：
   id 会在对象回收后重用，那会让一个新页面被误当成"已经装过了"而跳过。
4. 漏页告警改成**只在数字变化时说一次**，不再每 2 秒刷一屏。

**验证**（scratch Chrome，真·可信点击）：跨站导航后**人为把监听器整个删掉**
（`removeEventListener` + `delete window[registry]`）→ 点击**不记录**（0，符合预期）
→ 巡检回读发现标记没了 → 就地补装 → 再点击**记录成功**（1）。
**这条路径才是真正的保证**：它不依赖"我们猜对了原因"。

新增 6 条断言（`tests_publish` 112 → 118）。**19 套 / 1225 项全绿。**

#### 顺带回答用户的问题

> 「难道只有脚本开始监听之前就需要人工打开浏览器中的 fb page 页面才能实现监听吗？」

**不需要，也不应该需要。** 工具会给已开的每个页面装监听器、给新开的标签页装、
导航后补装、掉了再补装。**如果还需要你迁就它，那是工具的问题。**
唯一还需要你做的是**瞄一眼终端有没有在跳 `#N click`** ——
那是"现在到底有没有在记"的唯一可靠信号。

### 18.7 追加 · CR-66 · 「按 Enter 停止记录」并没有真的停，于是问答处死锁

用户实测：按 Enter 之后，20 个观察项一路回车跳过时**卡住不动**，
而屏幕上还在冒 `#18 click` … `#24 click`。

#### 根因：停止只停了巡检，没停记录

`stop_sweep.set()` / `sweeper.cancel()` 只关掉巡检任务，
**页面上的监听器和 CDP 会话都还在**。于是问答期间的每一次点击照样
`create_task(record(...))` 排队，而 `record()` 是**持锁**的、
单条最坏要十几秒（8 秒 Playwright 截图 + CDP 回退的三段超时）。
`set_observations()` 要拿同一把锁 —— 排在那一串任务后面，看起来就是卡死。
**队列还是无界的**：用户在问答期间每点一次，就多排一个。

处置：按 Enter 之后**真的关闸**——

1. `recording["on"] = False`，`receive` 直接丢弃后续事件；
2. 逐个 `session.detach()`，浏览器不再回送事件；
3. **顺序不能反**：先 `sleep(0.9)` 给最后一次 input 的 700ms 去抖留时间，
   再关闸，否则会丢最后一条；
4. 关闸发生在问观察项**之前**——那正是死锁的来源。

#### 顺带：那 20 个问题本来就不该挡在出口

用户的原话是「我根本不会去进行人工填写」。**这个抱怨是对的，而且重要**：
录交互要对着浏览器一步步走，量 UI 边界（画幅上下限、正文长度、排期窗口）
要去 UI 上反复试，**本来就是两件事、两个时间点**。捆在一起问，
结果只能是一路回车跳过——**那样填出来的是假数据，比空着更糟**。

所以拆开：

- **默认不问**（`run_probe(collect_notes=False)`）。录完打印一行提示：
  这些值不影响交互 dump，但 `--strict` 需要，随时能补；
- `--notes` 想就地问的仍然可以；
- **新增 `--fill-notes <dump.json>`**：不连浏览器、不录制，只给已有 dump
  补填观察项。已有值会显示出来，直接回车＝保留。
  写回时**只动 `observations`，交互记录一条不改**；
- 缺项时按 `compose._PROBE_REQUIRED_OBSERVATIONS`（**直接问 compose 要，
  不在这里抄第二份**）逐条列出还缺哪些，而不是让人以为可以 `--strict` 了。

⚠️ **这不是把红线放松了。** 那几个数字仍然必须是人亲眼看 UI 得到的，
`_validated_probe_dump` 的必填校验一个字都没动；变的只是**什么时候问**。

#### 又一个真机 bug：观察项里写中文会把整份 dump 写崩

补填测试时当场撞上：`UnicodeEncodeError: surrogates not allowed`。
Windows 上 `input()` 遇到本机编码解不出的字节会走 `surrogateescape`，
留下 `\udcaf` 这类孤立代理字符，
`json.dumps` + `write_text(utf-8)` 直接抛。
**而这些框本来就是让人写中文说明的**（"直接输入可以"、"列表出现新行"）。

处置：新增 `console_text()`——先原样试，不行就把代理字符还原成原始字节、
按 UTF-8 / GBK 依次重解，都不行才退到替换字符；`_write()` 再加一层
`errors="replace"` 兜底。**宁可看到几个 U+FFFD，也不能让整份 dump 写不进去。**
实测：用 GBK 字节（本机控制台真实形态）喂进去，
`直接输入可以` / `拒绝并提示` 全部原样写回。

> 写这一节的时候，**我自己那个临时脚本也被同一个字符搞崩了一次**，
> 而且因为它是 `write_text` 直接覆盖（没有先写临时文件再 replace），
> 把整份 `CODE_REVIEW.md` 截成了 0 字节 —— 靠 git 恢复的。
> `ProbeRecorder._write()` 用的正是"临时文件 + 原子替换"，**那个选择是对的**。

新增 16 条断言（`tests_publish` 118 → 134）。**19 套 / 1241 项全绿。**

### 18.8 追加 · CR-67 · 一次失败的记录把序号永久空掉，整份 dump 就废了

**发现方式**：整理交接文档时核对用户那份 Business Suite dump，
发现它 `sequence` 缺了 #19 / #21 / #24 / #27。
...
**先占号、再做可失败的事**。中途任何一次失败——截图抛异常、
`create_task` 的任务在进程退出时被取消、用户 Ctrl+C——
这个号就**永久空掉**，而且**没有任何办法补回来**。

而 `compose._validated_probe_dump` 明确要求
`item.get("sequence") != sequence`（从 1 起严格连续），
所以**一份内容完好的 dump 会被判为"交互序号不连续"而永远进不了严格模式**。

用户那份 24 条就是这么废的：CR-66 的死锁让他中断了进程，
几条在半路的记录死掉，留下四个空号。**数据在，截图在，但严格校验不认。**

#### 处置

序号改成 `len(self.data["interactions"]) + 1` —— **由已经落盘的条数推出来**，
不再维护独立计数器。失败的那一次不占号，下一次接着用同一个号
（截图文件同名覆盖，不留垃圾）。

**断言**：构造一个"第 2 次截图必抛"的假页面，连记 4 条 →
序号仍是 `[1,2,3,4]`，且 4 条一条不少（失败那条带 `screenshot_error`）。

#### 顺带说清楚：那份 24 条的 dump 现在怎么用

- ✅ **可以人工读、用来回填 `selectors.py`** —— 那本来就是 G1 产出的主要用途，
  24 张截图齐全、零截图失败，覆盖了 composer 的完整发帖路径；
- ❌ **不能当严格模式的凭证**（`finished_at` 也是 `null`，同样被中断所致）；
- ⛔ **不要手工改序号把它"修好"**：`sequence` 连续与 `finished_at` 完整
  这两条校验存在的意义就是证明记录没丢过，**改了等于把证据抹掉**。
  要严格模式，重跑一次干净的即可（死锁与序号 bug 都已修）。
...
## 19. CR-69 · P0 · G6c 的证据契约是照着想象中的 UI 写的（2026-09-01 第五轮）

**又是真机跑出来的，不是审查看出来的**，而当时离线测试 22 套全绿。
这一条比 CR-63~68 更值钱：坏掉的不是某一行代码，是**整套判据的形状**。

### 症状

用户录到第一份完整通过 v2 契约的 dump
（`publish_probe_20260901_054226_378622.json`，56 交互 / 71 快照 / 117 截图）。
`probe_signals --check` 报"推不出账号上下文 / 排期卡片"。

第一反应是"录漏了，让用户重录"。**那个反应是错的。**

### 真因（追进 dump 才看得到）

`--report` 把 dump 里真实存在的语义摊开之后：

| 契约要求 | 现实 |
|---|---|
| composer 上同一容器同时露出 FB Page 与 IG 帐号 | **46 张 composer 快照里 `neakasa.de` 命中 0 次**。渠道只有 `img 'Instagram'` 一个图标 |
| 一张 Planner 卡片内含时刻/正文/FB/IG/图片数五组独立子语义 | **FB 与 IG 是两个独立对象、两个 remote ID、两个详情弹窗**，不可能同框 |
| `range_regex` 解析 "start - end" 日期范围串 | 真实是 `heading 'September'` + `heading '2026'` **两条独立 heading** |
| `image_count_regex` / `media_image_role` | Planner 侧**零数量语义** |
| 正文是卡片内独立子元素 | 正文与时刻**在同一条 `link` 的可访问名里** |

**缺的东西不是没录到，是那个 UI 上不存在 —— 重录一百遍也不会有。**

### 处置

按现实重写契约（详见 `docs/PROBE_FINDINGS_20260901.md` 第四节）：

- 提交前只证明 FB（预览抬头 `heading h2`，**完整值**比较）；
- IG 改由提交后从 Planner 详情弹窗回读证明。
  **"两个渠道少任一个都不记 `scheduled`"这条保证没有放松**，只是挪了时机；
- 排期证据按**多张快照的并集**验 —— 条目与两个弹窗天然不同框；
- 图片数量硬闸从 Planner 搬到 composer 上传后数缩略图；
- `journal` 新增 `remote_ids`，每渠道一个远端 ID。

### 19.1 这一轮顺带抓到的四个同型 bug

**三个都是"拼接文本"造成的。** `_row_text` 把 `accessible_name` 与
`visible_text` 拼起来，而这两者在真实 dump 里**常常一模一样**：

| # | 现象 | 判据错在哪 |
|---|---|---|
| a | `"5 photos"` 被数成两处数字 → 唯一正确的图片数量行被判掉 | 在拼接串上 `findall(r"\d+")` |
| b | `"September"` 变成 `"September September"` → `strptime(text, "%B")` 永远失败 | 在拼接串上解析 |
| c | 一个时刻被当成两个 → 条目被判"分不清哪个是目标" | 按**匹配次数**判，而不是按**指向几个不同的时刻**判 |
| d | `schedule` 子串命中成功提示「Your post is **scheduled**」→ "提交按钮"被推成那条 toast | 措辞排序用子串而不是整词 |

**教训**：凡是要在单值上判的，就不能拿拼接串去判；
凡是"只能有一个"的判据，要问"指向几个"而不是"匹配几次"。

### 19.2 还有一条差点放行的

`token.casefold() in text.casefold()` 判账号 —— **`neakasa.de` 是
`neakasa.deals` 的子串**。改成 `(?<![\w.@])TOKEN(?![\w.@])` 按独立词判。

⚠️ 它仍然挡不住「目标名 + 后缀」（`Neakasa Deutschland Test`），空格是合法词边界。
挡那个的是**提交前**那道 composer 账号闸：比的是独立元素的**完整值**，多一个词就不等。
**两道闸各挡一半，缺一不可** —— 这一点写进了 `_token_present` 的 docstring，
免得下一个人看到"这个检查不严"就去改错地方。

### 19.3 给下一个人的一句话

**18.9 说"mock 掉的边界就是没被测到的边界"。这一轮把它推进了一步：
照着想象写出来的夹具，测的是你的想象，不是那个系统。**
旧夹具（`planner_semantics()` 那一套）自洽、全绿、且完全不对。
现在的夹具**逐字照抄真实 dump**，`docs/PROBE_FINDINGS_20260901.md` 里
每条断言都能回查到快照编号。**下次写夹具之前，先有一份真实 dump。**

---

## 20. CR-70 · P0 · 账号闸查的那条 heading，要等预览渲染才存在（2026-09-01 第六轮）

**这条又是真机跑出来的，离线 23 套全绿。** 第一次 G8 `--submit` 停在第二步：

```
[2/7] 核对登录态与目标主页 …
  发布失败：G2 登录态与目标主页
  原因：找不到 v2 已录证的当前发布账号上下文；为避免发错主页，自动提交在上传前停止：
        Locator.wait_for: Timeout 30000ms exceeded.
        waiting for get_by_role("heading", name="Neakasa Deutschland").first
```

### 20.1 不是环境问题，是调用顺序错了

已录证的 `composer_account_context` 是 **FB 预览面板里**那条
`heading 'Neakasa Deutschland'`，而**预览面板要有内容才渲染它**。
dump 里的时序是决定性的：

| 事件 | evidence_order |
|---|---|
| 点 `Add photo/video`（交互 #12） | 29 |
| **`heading 'Neakasa Deutschland'` 第一次出现** | **31** |

代码却在 `[2/7]` 等它，而上传是 `[3/7]` —— **闸在证据存在之前就查了**。
录制时人是先传图、预览才露出来，所以 dump 里有；真跑时顺序反过来，
空 composer 上等 30 秒也等不到。

### 20.2 一句话教训

> **录证据的顺序 ≠ 执行的顺序。**
> 一条定位"在 dump 里存在"只证明它**在某个时刻**存在，
> 不证明它**在你要用它的那个时刻**存在。
> `evidence_order` 一直都在 dump 里，只是没人拿它和执行顺序对过。

这与 18.9（mock 掉的边界就是没被测到的边界）、19（照着想象写的夹具）
是同一族：**离线夹具把页面当成静态的**，而真实 SPA 的语义随交互长出来。
离线测试用一个"什么都有"的假 page，所以永远发现不了这一类。

### 20.3 修法：挪到上传之后，不是放松

`publish/workflow.py`：`[2/7]` 改成只做弱核对（正文框在 + 页面文本里有目标主页名，
两者在空 composer 上都成立），**严格的 role 核对挪到 `[3/7]` 上传之后**。

**没有放松任何一道闸**：

- 上传素材到 composer **不会发布任何东西**；
- 闸仍然挡在**点提交之前**，发错主页照样发不出去；
- 而且此刻读到的是"即将提交的那一屏"，比空屏时更有说服力。

⛔ **不要为了"更早拦住"把它挪回上传前** —— 那里没有证据可读。

### 20.4 ⚠️ 一个很诱人但不能用的捷径

失败截图上，`Post to` 下拉框明明白白写着
**"Neakasa Deutschland and neakasa.de"** —— 一个控件同时证明两个渠道，
比 FB 预览那条 heading 好得多，而且空 composer 上就有。

**但它在全部 46 张 composer 快照里命中 0 次**，从来没被 dump 录到
（probe 只采样带 role 的元素，这个控件的值没进可访问名）。

**所以不能用**（全局红线 5：不许凭截图猜一个定位顶上）。
截图证明的是**文本存在**，不是**role 与 DOM 结构** —— 而 Playwright 要的是后者。
想用它，得重录一份能录到它的 dump。

⚠️ 顺带更正一处措辞：`PROBE_FINDINGS_20260901.md` 第一节说
"composer 上从头到尾没有 IG 账号名"。**准确说是"dump 里没有"** ——
界面上其实有（就在 `Post to` 里），只是没被采样到。
结论不变（不能拿它当定位），但别把它当成"UI 上不存在"。

---

## 21. CR-71 · P0 · 逐字符敲键把 @ 提及撕成两半（2026-09-01 第六轮）

**又一条真机跑出来的，离线全绿。** 修完 CR-70 之后，G8 走到 `[4/7] 填正文`：

```
  原因：正文回读与要填的不一致，**没有继续**。
  第 109 个字符起不同（期望 214 字 / 实际 226 字）
  期望…' sind so bereit!! ⏎⏎Wer ist neugierig? L'…
  实际…' sind so bereit!! ⏎⏎⏎Wer ist neugierig? '…
  （⏎ = 换行符；报出来的差异只有这一处多余换行）
```

### 21.1 报出来的和真实的破坏，差得很远

那条提示看着只是"多了一个换行"。**截图里的正文框才是真相**：

```
期望   … Bescheid 👇 ⏎⏎#Neakasa #NeakasaatIFA #IFA2026 #Cats #PetsHome
实际   … Bescheid 👇 ⏎@ifa.ber #Neakasa #NeakasaatIFA #IFA2026 #Cats #PetsHomelin
（⏎ = 换行符）
```

`@ifa.berlin` 被**撕成 `@ifa.ber` + `lin` 分别插到了两个地方**，
而正文第一段里那个 `@ifa.berlin` 还好好的 —— 等于凭空多出一份。

**多出来的换行只是第一处差异，不是主要破坏。**

### 21.2 根因：`keyboard.type()` 逐字符派发真实按键

Meta 的编辑器（Lexical 形态）在 `@` / `#` 上挂着提及与话题标签的 typeahead。
逐字符敲键会把菜单招出来，菜单异步改 DOM 并移动光标，
后续字符就落到别的位置去了。

原实现的注释写着"**不去躲它**——躲它就得猜菜单的定位，改成让回读闸去抓"。
**那个判断只对了一半**：回读闸确实抓住了（它工作得很好），
但抓住之后**这一步就永远过不去** —— 而几乎每一篇德语帖都带 `@` 或 `#`。
"能检测"不等于"能完成"。

### 21.3 修法：换掉输入通道，不是放松闸

`keyboard.insert_text()` 走 CDP 的 `Input.insertText`。
Playwright 自己的文档写得很清楚：

> Dispatches only `input` event, does not emit the `keydown`, `keyup` or
> `keypress` events.

没有按键事件，typeahead 就不会被触发；而编辑器仍然收到 `input`，
所以话题标签照常被 token 化（截图里那些蓝色高亮还在）。

两种写法**依次试，每种都当场逐字符回读**：

1. 整段 `insert_text`（零按键）；
2. 逐行 `insert_text` + 行末 `Shift+Enter`（防止整段插入时 `
` 没变成换行）。

⚠️ **回读闸一条都没放松**：两种都对不上就带完整差异停下、绝不提交。
重试是安全的 —— 这一步在提交之前，写坏了只是 composer 里的草稿。

⚠️ G5 往 `mm/dd/yyyy` 那个**普通 textbox** 里敲日期**仍然用 `keyboard.type`**，
那里没有 typeahead。`tests_publish.py` 的源码断言只查 `_write_caption` 一个函数。

### 21.4 顺带修掉一个让排查变贵的东西

`_diff_hint` 原来只打第一处差异前后各 20 个字符，
**恰好把真正的破坏挡在窗口外**，为看清它多花了一整轮真机。
现在**把期望与实际完整打出来**（正文上限 2200，代价可以忽略）。

> **一句话教训：诊断信息的窗口不能比故障小。**
> 截断的证据会让人去修错的东西 —— 这一次差点去修换行归一化。

### 21.5 离线怎么钉住

`tests_publish.py` 里给假页面加了 `on_key_type` 钩子，
**只在按键路径上生效**（`insert_text` 不触发）：

- 一条模拟 typeahead 打乱正文，断言 `fill_caption` 仍然填对 ——
  **改回 `keyboard.type` 这条立刻红**；
- 一条模拟"整段插入丢换行"，断言自动回落到逐行写法；
- 一条源码断言：`_write_caption` 里不许出现 `page.keyboard.type(`。

---

## 22. CR-72 · P0 · 分段时间输入上 Ctrl+A 选不中当前格（2026-09-01 第六轮）

**第三条真机跑出来的，离线仍然全绿。** 修完 CR-71 之后走到 `[5/7] 设排期`：

```
  原因：时刻回读对不上：期望 1 : 00 AM，UI 上是 '11⏎:⏎00⏎AM'。
        （⏎ = 换行符）
```

小时被写成了 **11**，帖子会排到 **11:00 AM 而不是 1:00 AM**。

### 22.1 根因：`_type_into` 假设 Ctrl+A 一定选中当前字段

原实现是「点一下 → Ctrl+A → 打字」。对普通 `<input>` 成立，
但 Business Suite 的时刻是**三个分段 `<input>`**（时 / 分 / AM-PM，
dump 里 `tag='input'`、交互 #40–43 与 #46–49 是 click→input 成对出现）。
在这类控件上 **Ctrl+A 选不中那一格**，于是新字符**追加**到旧值后面：

```
打开定时开关后 UI 预填的小时 = 1
程序要写 1  →  Ctrl+A（没选中）→ 敲 "1" →  变成 11
```

### 22.2 两个让它一路溜到最后的设计缺陷
...
try:
    ...
    return                       # ← 非 input 元素：直接放行，退路根本不跑
await target.fill(value)         # ← 跑完就返回，**不验证 fill 有没有生效**
```

真机上 `fill("1")` 跑过了，而最终回读**仍然是 11** ——
分段输入不接受整串替换。程序却以为自己写成功了。

**② 报错只给合成串，看不出是哪一格坏的。** 报的是
"UI 上是 `11 : 00 AM`"，人得自己去猜是小时格的问题还是整个控件的问题。
这和 CR-71 的 `_diff_hint` 是**同一个毛病**：诊断信息比故障小。

### 22.3 修法

- 新增 `_clear_field()`：Ctrl+A + Delete 之后**回读确认真的空了**，
  不空就 `End` + 逐个 `Backspace` 兜底（最多 12 次，够任何时刻字段）；
- `_type_into()` 改成**不做对错判定**，只"尽力写进去并把结果如实交出来"——
  判定留给调用方的**语义**检查（`_same_date` / `_same_time`）。
  ⚠️ 这一点很关键：UI 合法地把 `9` 显示成 `09` 不是错误，
  用裸字符串相等去判会把它误报成故障（原来的 `_type_into` 就有这个隐患）；
- 时刻整组回读失败时，**逐字段打出当前值**。

### 22.4 离线怎么钉住（以及差点又写出一条假绿灯）

给假页面加了 `segmented=True` 的 `FakeElement`：Ctrl+A/Delete 清不掉它、
打字追加、**`fill()` 不生效**。

⚠️ **最后那一条是补上去的，而且是决定性的。** 第一版只建模了前两条，
结果拿修复前的 `_clear_field` 去跑，假页面**没有复现 bug** ——
因为假 `fill()` 总是生效，把问题掩盖了。
真机的事实是：打字得到 `11` 之后 `fill("1")` 跑过了，**最终回读仍然是 11**。
照这个事实建模之后，修复前如期红：

```
时刻回读对不上：期望 1 : 00 AM，UI 上是 '11 : 00 AM'。
  逐字段当前值：小时='11'、分钟='00'、AM/PM='AM'
```

> **一句话教训：新写的回归测试，必须先拿掉修复跑一遍看它变红。**
> 不验证这一步，写出来的很可能是一条永远绿的断言 ——
> 那比没有测试更糟，因为它会让人以为这里被保护着。

### 22.5 三条真机 bug（CR-70/71/72）的共同点

| # | 表面 | 真因 |
|---|---|---|
| 70 | 找不到主页 heading | 证据在**上传之后**才存在，闸却查在上传之前 |
| 71 | 正文多一个换行 | 逐字符按键招出 typeahead，`@提及`被撕成两半 |
| 72 | 时刻差一个字符 | Ctrl+A 在分段输入上选不中，新值追加到旧值后 |

**全部是"离线夹具把页面当成静态、可整串替换的普通表单"。**
真实 SPA 的语义随交互长出来，控件是分段的、受控的、带 typeahead 的。
18.9 说"mock 掉的边界就是没被测到的边界"，19 说"照着想象写的夹具测的是想象"，
这三条把它推到第三层：**夹具的每一条行为都要有真机依据，
尤其是那些让测试变绿的行为。**

---

## 23. CR-73 · P0 · 每个渠道各有一套排期控件，程序只设了 Facebook 那一套（2026-09-01 第六轮）

**这是这条链上目前最危险的一个 bug**，也是唯一一个**内容全对、
主页全对、而时刻悄悄错**的。真机截图是决定性的：

```
Schedule
  Facebook    Sep 10, 2026    11:00 AM     ← 程序设的
  Instagram   Sep  1, 2026    06:23 PM     ← **没动过，是默认值（当天+此刻）**
```

**Instagram 那一篇会几乎立刻发出去。**

### 23.1 dump 里其实一直写着，只是被读成了别的意思
...
上一轮判读把这两轮读成了"人改了一次时间"，
`PROBE_FINDINGS` 里也照这个意思写着「日期框 | 交互 #37/#44」。
**真相是两个渠道各设一次。**

⚠️ 有意思的是：那两组控件**一张快照都没采样到**（composer 的 46 张里
`textbox 'mm/dd/yyyy'` 与 `spinbutton` 出现 0 次），只有交互记录到。
所以"从快照里找证据"这条路走不通，**证据在交互的时序里**。

### 23.2 修法

`set_schedule` 改成对 `locator_for(...).all()` 拿到的**每一组**都设一遍：

- 日期框与时间控件**数量必须相等**，否则失败闭合 ——
  ⛔ 不去猜哪个日期框配哪个时间控件，配错的后果和不设一样严重；
- 每一组各自回读校验；
- 返回的那行把**每一组都打出来**，人一眼能看到两个渠道排的是同一时刻。

⚠️ 这里**没有**新增任何选择器：用的还是 `schedule_date_input` /
`schedule_time_group` 那两条已录证的定位（`role='application' name='Time input'`
足够具体），只是不再截断成 `.first`。截图上那两个
`heading 'Facebook'` / `heading 'Instagram'` **没有**被拿来做定位（红线 5）。

### 23.3 顺带修掉两个

**① 小时用两种写法依次试**（`1` 和 `01`）。分段时间输入通常按固定两位消化
数字并自动跳格，只给一位时它可能还在等第二位。哪种对只有真机知道，
所以两种都试、每种都当场回读。

**② 我自己引入的一个运算符优先级 bug。** 上一轮改错误消息时写成：

```python
raise PublishStepError(
    "…期望 %d : %02d %s，UI 上是 %r。" + chr(10) +
    "  逐字段当前值：%s"
    % (hour12, local.minute, meridiem, rendered, "、".join(detail)))
```

`%` 的优先级**高于** `+`，于是格式化只作用在最后那一段字符串上，
5 个参数喂给 1 个占位符 → `not all arguments converted during string formatting`。
**错误处理路径本身抛异常**，把真正的失败原因盖掉了。
已用 AST 扫过全仓库同型写法（`字符串 + (字符串 % ...)`），**零命中**。

> **一句话教训：错误处理路径也是代码，而且它天生缺测试** ——
> 它只在出事的时候跑，而出事的时候没人有心情检查它对不对。

### 23.4 离线怎么钉住

假页面加了 `channels=2`：每个渠道各一套日期框 + 时间控件。
按 CR-72 立的规矩，**先拿掉修复跑一遍确认它真的红**：
...
第二个渠道**完全没被碰过** —— 正是真机上 IG 停在默认值的那个形状。
另加一条：日期框与时间控件数量对不上时失败闭合。

---

## 24. CR-74 · P1 · `.all()` 不等待，而排期区是开关打开后才异步渲染的（2026-09-01 第六轮）

**这条是我自己在修 CR-73 时引入的回归。** 报的是：

```
排期控件配不成对：日期框 0 个、时间控件 0 个。
```

### 24.1 换掉 `.first` 的时候把"等待"一起丢了

原代码：

```python
date_input = locator_for(page, "schedule_date_input").first
await date_input.wait_for(state="visible", timeout=_ms(timeout))   # ← 会等
```

改成枚举全部之后：

```python
date_inputs = await locator_for(page, "schedule_date_input").all()  # ← 不等
```

**`locator.all()` 立刻返回当下匹配到的元素，不轮询、不等待。**
而排期那一块是**打开定时开关之后才异步渲染**的，
于是在它还没出来的那一刻枚举，拿到 0 个。

> **一句话教训：`.first` 后面跟的那句 `wait_for` 不是装饰。**
> 把 `.first` 换成 `.all()` 时，等待语义**默认消失**，而且失败得很像
> "这个东西不存在"，不像"我来早了"。

### 24.2 顺带补上一个更隐蔽的：只等到"第一个"是不够的

Facebook 那一行先渲染、Instagram 那一行晚一拍时，枚举会拿到 **1 个**，
然后**静默地只设一个渠道** —— 那正是 CR-73 的后果（IG 停在默认值 = 立刻发）。

所以 `_settle_pairs()` 会等到两组定位的条数**稳定且相等**再枚举。
⛔ 它自己不抛异常：条数最终对不上，由调用方那条"配不成对"报，
那里的错误信息完整得多。

### 24.3 离线怎么钉住：先把假 locator 改成**惰性**的

**这一条改动本身比断言更重要。** 假页面原来把 locator 的命中结果
**在构造时就冻住**：

```python
FakeLocator(self, _walk_role(self.elements, role, name, exact))   # 命中结果被冻住
```

于是"等一会儿元素才渲染出来"这件事在假页面上**根本不可能发生** ——
这一类 bug 无论写多少断言都测不到。

改成每次用到时重新解析（真实 Playwright 的 locator 本来就是"怎么找"、
不是"找到的东西"），再加 `deferred` 标记模拟异步渲染，
按 CR-72 立的规矩拿掉修复跑一遍，**假页面复现了真机一模一样的报错**：
...
另加一条源码断言：`set_schedule` 里 `wait_for` 必须出现在 `.all()` 之前。

### 24.4 CR-70 ~ CR-74：五条真机 bug 的总账

| # | 表面 | 真因 | 夹具为什么测不到 |
|---|---|---|---|
| 70 | 找不到主页 heading | 证据在上传**之后**才存在 | 假页面上元素一直都在 |
| 71 | 正文多一个换行 | 逐字符按键招出 typeahead | 假键盘不区分按键与 insertText |
| 72 | 时刻差一个字符 | Ctrl+A 在分段输入上选不中 | 假 `fill()` 总是生效 |
| 73 | IG 排到了"立刻" | 每个渠道**各有一套**排期控件 | 假页面只造了一套 |
| 74 | 排期控件 0 个 | `.all()` 不等待 | 假 locator 的命中结果被冻住 |

**五条全部是"假页面比真页面简单太多"。** 每修一条，都是把假页面往
真实 SPA 的方向补一格：元素会晚出现、控件是分段的、键盘有两种通道、
一个渠道一套控件、locator 是惰性的。

> 18.9 说"mock 掉的边界就是没被测到的边界"；
> 19 说"照着想象写的夹具测的是想象"；
> 这五条给出第三句：**夹具的每一处"比现实简单"，都是一个等着被真机撞开的洞。**

---

## 25. CR-75 · P1 · Planner 的「就绪信号」在日历还在转圈时就已经渲染好了（2026-09-01 第六轮）

**第六次 G8 终于提交成功了** —— 成功信号看到了，两个渠道的时刻也都对
（`9/10/2026 01:00 AM ／ 9/10/2026 01:00 AM`）。但最后一步栽了：

```
submitted_unverified  G6c 内容日历回读
  内容日历里找不到同时匹配目标时刻与完整最终正文的卡片；
  禁止自动重试，必须人工确认远端是否已经排期
```

**帖子是真的排上了**（用户在 Business Suite 上看到了，随后手工删掉）。
是回读跑早了。

### 25.1 截图是决定性的

`state/publish_attempts/<attempt>_scheduled.png`：
`Planner` 标题、`September 2026`、`Week`/`Month` 全都在，
**日历区域中间一个大转圈**。

而 `planner_loaded_signal` 推导出来的是 **`heading 'September'`** ——
它属于页面外壳，**在数据加载完成之前就渲染好了**。
契约里明明写着"必须是 React 数据完成后的语义，URL/页面骨架不能证明数据就绪"，
但从那份 dump 里机械推导出来的这一条**并不满足它自己的要求**。

### 25.2 这条闸的另一侧更危险

`_planner_cards` 同时给**提交前的基线检查**用（`[0/7]` 那一步，
确认远端没有同槽/同文案/同素材的卡片）。那一侧把"还在加载"误读成
"远端零占用"，等于**把防重复发布的闸悄悄打开**。
这次只是漏判了一条自己刚发的帖；换个时机就是发重。

### 25.3 修法：等到真的出现能解析出时刻的条目

⛔ 不能等"任意 link 出现" —— 下游那句注释早就写着
「日历上的 link 不止排期条目一种（导航、侧栏都是 link）」。

所以判据用**已录证的 `datetime_regex`**：能从可访问名里解析出时刻的 link
才算日历条目。这不是新选择器，是把已有的证据用在等待上。

- 预算 `_PLANNER_ENTRY_BUDGET = 20` 秒，不是整个 `ui_timeout` ——
  **空日历会真的等满这一段**，设成 30 秒等于每次发布先干等半分钟；
- 等满之后原样返回当下读到的（通常是纯导航 link），
  由调用方按内容筛出零条 —— **真的空日历仍然读作空**。

### 25.4 六次真机、六条 bug 的总账
...
| 73 | IG 排到了"立刻" | 每个渠道**各有一套**排期控件 | 假页面只造了一套 |
| 74 | 排期控件 0 个 | `.all()` 不等待 | 假 locator 的命中结果被冻住 |
| **75** | **刚发的帖回读不到** | **就绪信号早于数据** | **假页面没有"加载中"这个状态** |

**六条全部是"假页面比真页面简单太多"。** 每修一条都是把假页面往真实 SPA
补一格：元素会晚出现、控件是分段的、键盘有两种通道、一个渠道一套控件、
locator 是惰性的、**页面有"骨架好了但数据没好"的中间态**。

> 18.9：mock 掉的边界就是没被测到的边界。
> 19：照着想象写的夹具测的是想象。
> 70~75：**夹具的每一处"比现实简单"，都是一个等着被真机撞开的洞。**
> 而它们全都撞在同一类东西上 —— **时间**：什么时候出现、什么时候可写、
> 什么时候算加载完。离线夹具天然没有时间维度。

---

## 26. CR-76 · P1 · 定时开关点下去之后没等它翻（2026-09-01 第七轮）

```
点了 'Set date and time' 但它没有被打开（回读仍是关）。
```

**用户第一反应是"上一轮明明成功了，是不是你改坏了"。查了 diff：没有。**
`git diff 5b5f081..HEAD -- publish/business_suite.py` 里开关那一段是
**未改动的上下文**，这一轮所有编辑都在它后面。

### 26.1 它一直是个竞态，上一轮只是撞对了

```python
if not await switch.is_checked():
    await switch.click(timeout=_ms(timeout))
if not await switch.is_checked():        # ← 中间一点都不等
    raise PublishStepError(...)
```

React 受控开关的 `aria-checked` 是**状态回来之后**才翻的，
`click()` 返回时往往还没翻。同一台机器上有时过、有时不过 ——
这正是竞态的特征，而竞态最擅长的就是让人去怀疑上一次改动。

> **一句话教训：「上次能跑」不是「这段代码是对的」的证据。**
> 竞态在成功的那一次和失败的那一次里是同一段代码。
> 先查 diff 再下结论 —— 但 diff 干净**不等于**没 bug，只等于不是新引入的。

### 26.2 修法：先等，再补点，顺序不能反

⚠️ **直接连点两下会把已经打开的开关又关回去**，而"关回去"的表现和
"根本没打开"一模一样 —— 那种错最难查。所以是：
点 → 轮询等 `aria-checked` 翻（预算 8 秒）→ 还没翻才补点一次 → 再等 → 放弃。

失败信息里带上 `aria-checked` 的实际值，并提示先怀疑"点到的不是那个开关"：
真机截图上 `Schedule` 下面还有 `Share to`（Facebook story / Threads）等别的开关。

### 26.3 第七条，同一个族

| # | 表面 | 真因 |
|---|---|---|
| 70 | 找不到主页 heading | 证据在上传**之后**才存在 |
| 71 | 正文多一个换行 | 按键招出 typeahead |
| 72 | 时刻差一个字符 | Ctrl+A 在分段输入上选不中 |
| 73 | IG 排到了"立刻" | 每个渠道各有一套排期控件 |
| 74 | 排期控件 0 个 | `.all()` 不等待 |
| 75 | 刚发的帖回读不到 | 就绪信号早于数据 |
| **76** | **开关点了没开** | **点击返回 ≠ 状态已翻** |

**70、74、75、76 都是同一件事：把"我做了这个动作"当成了"这个动作已经生效"。**
浏览器自动化里这两者之间永远隔着一段异步，而离线夹具里它们是同一瞬间。

假页面第七格：**开关的状态可以晚几拍才翻**（`checked_lag`）。
按规矩拿掉修复跑一遍，如期复现真机那句报错。
