...
填写后和提交前共用日期、时间语义核对：日期支持既有美式输入、英文月份及 ISO 值；时间支持 input 值和可编辑字段文字，不强求容器再重复时间，允许补零/空白/大小写等同值变化。填写日期和最后时间字段后 Tab 失焦，再读实际值；日期确实回退或变化仍停止。错误分清未知日期与实际不符，附确认值、时区和页面值。隔离浏览器覆盖原样 09-30 23:00、中午/午夜、跨日/月/年、控件失焦回退及未知日期；已有时区偏移、DST、发布锁和单次提交约束保持。验证、复审与保全见[本次汇总](../state/g8-schedule-form-20260924T081858Z/verification.json)，真实排期仍待服务机更新后按当前冻结内容确认。
