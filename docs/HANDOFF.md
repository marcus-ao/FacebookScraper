...
**失配定位与提前核对：离线通过。** 提交校验按正文、帖子编号、渠道、来源账号、来源指纹和版本列出失败项，v1 来源失配补充图片链接提示。在访问月历前只读核验；月份读取后仍重新核验并只在通过后绑定时刻。旧指纹、防重、发布锁和追加式 journal 保持原判据。[七项定向验证](../state/offline-validation-20260924T070651Z/results.json)覆盖批准、人工单篇排期、操作状态、快照恢复、来源版本、原排期补验和 hygiene；含真实临时归档中的 v1 换签名拒绝、显式解除/重冻到 v2、旧文件保留，以及月历读取期间来源变化再次拒绝。[独立只读复审](../state/g8-snapshot-submission-20260924/review.json)无阻塞意见。外部读取/提交用替身，没有真实排期；修复后的服务机提示与本帖重新冻结仍待用户操作。
