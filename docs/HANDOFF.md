...
聚合 Post 现按初始加载、FB、IG 各保留 40 条被动响应采集额度，整页最多 120 条，重访不重置。切换前同步生产采集器和可选探针的视图；视图只分配额度，身份仍须由原对象、成员媒体 ID 和 owner ID 关联。原账号、正文、独立分钟及缺渠道拒绝规则保留。探针输出各视图采集/丢弃数量，并补齐 `instagram_post` 的脱敏身份结构。每响应 4 MB、5 秒读取限制保留，不增加主动请求、不增加人工配置；这项修复仍需真实 Post 和月历复验，不能升级 G8。
