...
服务机待提供每渠道一个原 scheduled attempt、绑定快照与有效渠道资产记录，以及同一 remote ID 的详情结构、独立总数/末项、各位置实际字节与轮播加载关系；优先补验已有多图对象。没有合格对象才按 [MANUAL_STEPS §16.3–§16.4](MANUAL_STEPS.md#163-走一遍完整的人工流程) 展示具体冻结内容并取得确认，不能为补证重复排期。本分支已接入复查时最新 main `6949a10`，保留其 UI/历史筛选、列表占位及月历来源关联改动；本次真实样本确认仍需另展示柏林时刻，不恢复主干已撤下的前端时区显示。
