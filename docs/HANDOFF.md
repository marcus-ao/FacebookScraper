...
**服务机离线表单测试启动失败（2026-09-25）：离线通过，服务机复验待执行。** 用户补充的 `offline-validation-20260925T104935Z/tests_final_form.log` 确认 13 个用例在 `asyncSetUp` 找不到 `chromium_headless_shell-1234`，未执行表单断言；初始化失败跳过原 `asyncTearDown`，遗留 Playwright 驱动随后触发 Windows 管道清理异常。测试现与生命周期夹具一样使用既有 Chrome 程序的无登录临时会话，每取得一项资源立即登记异步清理；启动、上下文、页面初始化失败均清理已取得资源。离线入口显示首尾错误和完整日志路径，不再只显示最后 3500 字符。本机空浏览器缓存下[五脚本验证](../state/offline-validation-20260925T105504Z/results.json)通过，缺失 Chrome 路径的真实驱动故障注入保留原失败且无退出清理异常，见[诊断与证据](../state/g8-final-form-startup-20260925/diagnosis.json)。此修复仅涉及测试与诊断，不改变生产浏览器、表单校验或 G8 状态，不能用本次测试错误解释前一天真实 9223 整窗退出。
