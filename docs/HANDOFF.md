...
**同日多帖的详情读取与排期通知（2026-09-26）：待真实联调。** 用户反馈 `122128135023379375` 的 `2026-09-30 23:00 +08:00` 已恢复排期状态；随后 `122125865115379375` 的同日 `17:30` 收到成功信号但回执仍不完整，飞书也未及时通知。普通同日双卡夹具在修前即可区分身份，不能仅由截图断言两条卡片互相干扰。已复现的读取缺陷是详情标题/日期先出现，正文和 `See more` 延后到达时过早判缺失；现在限时等正文自身稳定并展开，仍核对该卡时刻、账号、独立编号和完整冻结正文，不等待照片后的评论进度条。
...
定向后端、前端测试与构建通过，复现入口及各批原始日志索引见 [verification.json](../state/g8-same-day-notifications-20260926/verification.json)。全部为隔离数据、合成页面或模拟机器人；未发送真实群消息、未重新排期。服务机的只读 HTTP 访问断开，已向用户索取新失败的逐条月历诊断与发件箱状态；这份数据尚未返回，不能将已修复的读取时序缺陷写成本次真实失败的已确认根因，也不关闭 G8。
