...
| **K 组**（`feat/image-de`） | **离线实现已完成，真实请求仍为零**：K0/K2/K4/K5/K6 已验收。真实预演显示当前最新 3 篇为 1/0/5 共 6 图（上游参考约 US$1.31），三篇都没有当前 `text_de`，但零图片帖无需翻译，实际只缺有图的两篇（按当前 usage 离线外推约 US$0.038）；现有 F `--limit 2/3` 实测会选最老帖子，不能精确补目标，须先获授权增加精确选帖或提供这两篇已审校译文。之后 K1 `--check`/图片批次须获费用确认，再用 K8 做德语审校并据真实 dHash 分布定阈值（当前 `-1`） | F 两篇当前译文与精确选帖；K1/K7/K8/K9；G8 |
