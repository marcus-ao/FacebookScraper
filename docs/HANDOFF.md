...
>
> **最后更新：2026-08-30**（A/B/J 组完成、解析器缺陷修复、归档重建、输出编码修复；
> **增量方案已定案为 B：登录态 + CDP 附着**，C 组已按 B 重写，可以直接开工）
...
**A 组和 B 组全部完成，J 组（归档布局）也完成了。数据是干净的、可以往下用。**

| 卡点         | 状态                                                                     | 阻塞了谁            |
| ------------ | ------------------------------------------------------------------------ | ------------------- |
| ~~A 组~~     | **全部完成**：账号名已填、小号已登录、回填已跑通                          | —                  |
| ~~B 组~~     | **全部完成**：解析器缺陷已修、归档已离线重建并验收                        | —                  |
| ~~J 组~~     | **全部完成**：每帖一个文件夹 + `index.html` 总览                          | —                  |
| **C2 → C7**  | ⬅️ **现在做这个。** 增量方案已定案为 B，C 组已按 B 重写完毕              | D3、E 组            |
| **F 组验收** | 需用户提供网关 base_url / 模型名 / 鉴权风格，并配好 API 密钥             | F1/F2/F3 的真实验收 |
| **G1**       | 需用户用**另一个 profile** 登录有 DE 发布权的账号，手工走一遍定时发帖    | G 组全部            |

### ⚠️ 接手第一件事：增量方案已经从"登出"改成"登录态"了
...
**你的第一个任务是 C2**（Instagram 登录态增量）：骨架照抄
`routes/backfill.py`，但不要人工介入、不要滚到底。
`routes/delta.py` 里现有的登出实现**保留不删**——端点若重开，
...
**离线测试基线：10 套 400 项检查，全绿**（在 stdout 被重定向的条件下也全绿，
...
C2–C7（**已按方案 B 重写完毕，可以直接开工**）、D3（检查接入增量）、
E 组（定时调度）、G 组（UI 发布）、H 组（API 验证，缺 Token 不阻塞）。

⚠️ **C7「累积风险缓解」不许跳过。** 它的参数已经以注释形式预置在
`config.toml` 的 `[delta]` 里（留成注释而不是真值，是为了避免
"改了不生效的假承诺"——项目里因为 `target_lang` 犯过一次）。
落地时把注释打开并让代码真正读取它们。

⚠️ **D 组还有一个待校准项**：`[integrity].alert_after_quiet_days = 4` 是按
"日均约 1 帖"设的，但实测该账号**最近一个多月没发新帖**（IG 最新一帖
2026-07-15）。这个阈值不重设的话，每个月都会误报。
...
- **任务 C3（Facebook 登出增量）有实质失败的可能。** FB 对登出访客大概率
  只给服务端渲染的 HTML。该任务已写成"三种结果任一种只要结论明确即算完成"，
  包括"完全被墙 → FB 增量降级为定期人工触发回填"。**不要在这一项上硬做。**
- ~~**Instagram 登出增量（C2）把握较大**，`web_profile_info` 端点登出可用。~~
  **已被真实证据下调**：2026-08-29 的首次真实探针即返回 429（见上面的 C2 小节）。
  这句原判断写的时候还没有任何真实证据。C2 现在与 C3 同属"可能不成立"一档，
  只是尚未定论。
