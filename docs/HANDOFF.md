...
独立分支 `codex/publish-probe-g1`；主检出提供
`state/publish_probe_20260920_100921_378853.json` 及同名 `_screenshots` 目录。
录制来自服务机 `C:\Users\admin\.fbscraper-publish`、9223，包含 232 条交互、204 条快照，
正常停止且 final 截图存在。用户确认没有另外的实测笔记，只采用录制可证明的事实。

两个代码问题已用隔离文件与 Chromium 复现并修复：旧绝对截图路径 `D:\Code\...` 使完整搬来的证据
被判缺失；录制器在遮蔽输入值时同时清掉 `aria-label`、`aria-labelledby`、关联 label。
截图现在仅从 dump 旁的同名直属目录核对，保留 JSON 原字节；旧主机路径不作为读取回退。
缓存命中仍复核 final 截图，缺图不能沿用成功。静态名称与输入值分开采集；正文、日期、时间值及
含可编辑后代的祖先文本仍排除。修复不能恢复旧 dump 已丢失的名称，须在服务机补录相应控件。

| 录制支持的事实 | 可复核位置 | 证明范围 |
|---|---|---|
| IG 最多 10 张图 | 快照 117/118 与对应截图 | 有明确超量拒绝，不证明边界画幅 |
| IG 最多 30 个标签 | 快照 108 与对应截图 | 有明确超量拒绝，不证明正文长度 |
| IG 排期提示 20 分钟至 29 天 | `187_click.png`、`semantic_124_periodic.png` | 截图中的约束提示；不是可定位语义，也不外推 FB 窗口 |
| FB 账号、Schedule 点击、成功提示 | 快照 17、交互 205、快照 136 | 三项信号可回查，不表示全链路通过 |
| FB 远端详情 ID `2059528092104126` | 快照 184 与对应截图 | UI 显示 2026-09-30 17:30；时区未核实，不能推算柏林时刻 |

**生产发布仍为待真实联调。** `observations` 是空对象；缺 UI 时区、画幅上下限及拒绝行为、正文长度
与计数方式、边界输入行为。录制无 IG 排期详情，FB 成功截图中的 Facebook Story 开关为开启，
不能视为仅 Feed 验收。没有本次 `channel_controls.json` / `planner_controls.json`；远端图片读取适配器
仍未实现，截图多图不等于冻结图片 SHA 与顺序已核验。现有录制不能替代这些条件。

信号工具现在同时列出 G1 缺项，不再将五项信号通过描述为可以上线；补录指引采用两篇独立单渠道样本。
`--set-note` 缺 `--fill-notes <dump>` 时直接拒绝，避免旧手册命令静默忽略笔记并启动浏览器录制。
操作入口与最小补证清单见 [MANUAL_STEPS §8](MANUAL_STEPS.md#8-录制单渠道-business-suite-证据)。
修复及隔离回归为 **离线通过**；没有连接真实 Chrome、登录、发帖、排期或改写业务账本，未修改生产签字配置。
真实文件重放及验证汇总保存在本工作树 `state/publish-probe-g1/`，原始录制保留在主检出 `state/`；
两者不随 Git 推送，清理前保全。

[定向回归](../state/offline-validation-20260920T025058Z/results.json)共 7/7 脚本通过：发布、发布 v2、
单渠道选择、pipeline 预检、运行状态、hygiene，以及新增录证契约 8 项（含隔离 Chromium）。
[真实录制只读重放](../state/publish-probe-g1/replay.json)核对 379 张被引用截图，原始 JSON 与图片哈希未变；
账号、提交、成功三项信号回查通过，IG 详情缺失使其余两项保持失败。
原始 JSON SHA-256 为 `5403e869bcb9bb12b7fa23b52ac4792e6eb12501fc93dc33b860276d19dd1b2e`。

后续核对补齐最终因果链中的同一截图解析规则：v2 结构通过后，不再在链尾退回旧机器的绝对路径。
新增完整链回归覆盖迁移后通过与删除 final 后拒绝；它使用隔离夹具，不能代替真实账号提交。
当前验证器仍要求同一 dump / 页面会话，独立补录不能自动合并，操作边界见手册第 8 节。
[补充定向验证](../state/offline-validation-20260920T030124Z/results.json) 3/3 脚本通过，
其中录证契约 9 项，另含发布 v2 与 hygiene。
