...
> 3. **下一步的顺序见下面「🚦 接下来做什么」那一节。** 一句话版本：
>    **代码侧先做 L0b（`pipeline status`）再做 L0c（死人开关）**，
>    然后用户装计划任务（L0a）；**发布那条线只等 G1 那 20 分钟**，
>    没有任何代码能替代。⚠️ **L0 的执行顺序 2026-08-31 改过**
>    （`PIPELINE_PLAN` 10.1），**不要按任务书的 a/b/c/d 排版顺序做**。
...
5. **`docs/CODE_REVIEW.md`** —— 已实现代码的审查记录（当前至 **CR-65**）。
   ...
   **第 17 / 18 节（CR-63、CR-64）是用户实跑撞出来的，不是审查看出来的**——
   离线全绿，而那两条路径在真实环境里**从来没成功过一次**。
   它们的共同点写在 18.6：**mock 掉的边界，就是没有被测到的边界**。
   CR-64 尤其贵：G1 recorder 全程一句提示都没有，用户走完整个发帖流程
   才发现 dump 是空的。
...
## 🚦 接下来做什么（2026-08-31 第七轮定的顺序）

**先分清两条终点线**，它们互不阻塞，可以并行推进：

| 终点线 | 卡在哪 | 谁能动 |
|---|---|---|
| **「能自动跑」** | 计划任务没装（L0a）；装了也没人看着（L0c 没写） | 代码侧现在就能动 |
| **「能发出去」** | **G1 的真实 DOM 探查** —— 卡着 G2–G9 全部九项 | ⛔ **只有用户能动** |

### 代码侧（不依赖任何人，现在就能开工）

**① L0b `pipeline status`** —— 只读对账，零网络零花钱零写盘。
放第一是因为它是**验证后面每一步有没有生效的仪表**，而且零风险。
基线数字已实测好，直接对答案：**可发 470（FB 27 / IG 443）· 待译 462 · 待调图 470**，
另有 597 篇纯视频/无图进不了发布口径。
⚠️ `待发布` 那一列在 G6 落地前是假的（判据文件 `state/published.jsonl` 还不存在），
**要么标注清楚，要么先不打印**。

**② L0c 死人开关** —— **必须赶在用户装计划任务之前落地**。
任务书原来的顺序是先装再补，第七轮改了（`PIPELINE_PLAN` 10.1）：
装计划任务是用户 5 分钟的事，写死人开关是半天的事，
**把便宜的那件放后面，就不会出现「自动化已经在跑、但没人能确认它还活着」的窗口**。
而"没跑"和"跑了但没事做"在输出上一模一样——这个项目为同型失效吃过一次亏
（合作帖静默丢 263 篇）。

> 这两项顺带还掉一笔债：实测**没有任何代码在读 `[pipeline]`**，
> `main` 上那四个键现在是死旋钮（CR-40 立过规矩不许有）。
> `dead_man_days` 归 L0c，两个 budget 键归 L1b。**在接上之前别再往里加键。**

### 用户侧（三件，都不要替用户按）

**③ L0a 装计划任务** —— 5 分钟，`MANUAL_STEPS` 第 9 步。**做完 ①② 之后再装**，
这样它从第一天起就是可观测的。装上那一刻起每天真的会去访问一次社媒。

**④ G1 真实 DOM 探查** —— 约 20 分钟，`MANUAL_STEPS` 第 11 步。
**这是真正的关键路径**：`publish/selectors.py` 是空的、`business_suite.py`
五个函数在碰 `page` 前就抛 `ProbeRequired`——**那是对的（红线 5）**，
而解开它的钥匙只在用户手上。在它完成之前，G2–G9 一项都动不了。

**⑤ K9 图片费用确认** —— 11 张 × high ≈ **US$2.4**，`MANUAL_STEPS` 第 10b 步。
⚠️ **它买的不只是 11 张德语图**：`config.toml` 现在 `dhash_max_distance = -1`，
也就是**「模型把整张图重画了」这道闸目前是关着的**，
而它只能用真实产出标定——**不跑 K9，这道闸就一直关着**（K5 的做法是对的，
没有真实样本时不许瞎填一个数）。

### G1 落地之后

**⑥ L0d 价格表** —— 46 种金额串已经抓好（`PIPELINE_PLAN` 14 节 L0d 里列着）。
⚠️ **第七轮把它从"可以先于 G 组"改成"排在 G1 之后"**：
`[publish.price_map]` 是 TOML 子表、必须在 `[publish]` 段最后，
而 G1 还要往 `[publish]` 写值；先插子表就留下"之后新加的标量键静默掉进表里"的窗口。
延后零代价——价格替换发生在发布前，而发布本身被 G1 卡着。

**⑦ L1a `pipeline run`** —— 对账器主干。等到有一条真能跑通的发布阶段可驱动再做。
