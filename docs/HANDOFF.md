...
>
> **最后更新：2026-08-30（夜）**（A/B/J 组完成、归档干净；
> **C 组代码与离线测试完成，等用户跑一次真实验收**）
...
**C 组（每日增量，方案 B）代码与离线测试也已完成，卡在用户跑一次真实验收**。

**你的第一个任务取决于用户带来了什么**，见下面「现在卡在哪」：
用户跑完实测就收尾 C 组并做 D3；没跑就先做 D3（不依赖真实抓取）。
...
| ~~C7~~       | **完成**（它的四条验收本来就是 mock 验收）                                | —                  |
| **C2–C6**    | 代码 + 离线测试完成，**标"进行中"**：验收要对真实账号跑一次抓取           | C 组合回 main       |
| **用户实测** | ⬅️ **等这个。** `MANUAL_STEPS.md` 第 8 步，约 5 分钟                     | C2–C6 勾选、E 组    |
| **D3**       | ⬅️ **不等用户，可以直接做。** 阈值已定：FB 7 天 / IG 21 天               | —                  |
...
- **跑了且正常** → 把 C2–C6 勾上（完成行里写实际数字），C 组合回 `main`
  （`git checkout main && git merge --ff-only feat/delta-logged-in`），然后做 E 组。
- **跑了但报错** → 按 `MANUAL_STEPS.md` 第 8 步那张故障表定位。
  如果是"解析器与真实结构不符"，`archive/*/_capture_delta_*.json` 里有原始响应，
  **离线改，不要让用户反复跑**（这是 B 组那次血的教训）。
- **还没跑** → 别催也别自己跑（每跑一次就是一次真实露面）。
  先做 **D3**，它不依赖真实抓取，且方案 B 之下它是"悄悄坏掉"唯一的出口。

### D3 的阈值已经定了，别再自己想一个

用户 2026-08-30 拍板：**`alert_after_quiet_days` 按平台分开，FB 7 天 / IG 21 天。**
配置写法与 `[delta].quiet_days_before_slowdown` 一致，用**内联表**
（`{ facebook = 7, instagram = 21 }`），避开 TOML 子表的排序陷阱。
依据（按归档实测）：FB 历史上最长的正常间隔 5.9 天；IG 六年里 >14 天的正常间隔
出现过 19 次。详见计划 D 组头部那张表。

### ⚠️ 一条被文档反复引用的错误事实，已纠正

旧文档里多处写着"该账号实测一个多月没发新帖，天天全速跑没有收益"——
**这句话只对 Instagram 成立。Facebook 是日更的**（中位间隔 1.0 天，
最新一帖 2026-08-25）。它原本会推出两个错误结论："增量没什么用"、
"一个阈值就够"。C7 的降频阈值和 D3 的告警阈值都按平台分开，根因就是这个。
...
### 归档现在长什么样（2026-08-30 重构后）

```
archive/in_neakasa.tech/
  index.html          ← 双击就能看完整个账号的历史（756 篇，按时间倒序）
  manifest.jsonl      ← **派生索引**，可用 tools.layout reindex 重建
  _rejected.jsonl     ← 被丢弃的 266 条他人帖，留痕不静默丢
  _orphan_media/      ← 重建后无主的 536 个媒体文件（移动不是删除）
  posts/
    2026-07-15_1434_3941857863710867472/
      post.json  text.txt  01.jpg 02.jpg 03.jpg 04.jpg 05.jpg
...
真实数字：**Instagram 756 篇（747 篇有正文）、Facebook 46 篇（45 篇有正文，
其中 18 篇含视频）**。图片 IG 648 张 / FB 70 张，全部 1080px 上下。
...
| **增量** | `routes/delta.py` _(方案 B 待建)_ | **登录态**（同一小号）+ CDP 附着 | 每天 | ⚠️ **累积性敞口** |
...
FacebookScraper/
  ...
  publish/                 ← 待建（G 组）
  tests/                   11 套 482 项检查，全绿（重定向输出下也全绿）
...
| **C2–C6**    | 代码 + 82 项离线断言完成，**但未勾选**——验收要真实抓取，见上 |

**离线测试基线：11 套 482 项检查，全绿**（在 stdout 被重定向的条件下也全绿，
