...
>
> **最后更新：2026-08-29**（A/C1/D1/D2/F 组完成、已实现代码二次审查后）
...
调试、跑通。**当前卡在需要用户人工介入的地方**（见下面「现在卡在哪」）。
...
| 卡点 | 需要用户做什么 | 阻塞了谁 |
|---|---|---|
| **A5** | 提供真实的 FB Page 用户名和 IG handle，填进 `config.toml` 的 `[targets]` | B 组全部 |
| **A3 / A4** | 双击 `scripts\start_chrome.bat`，人工登录抓取小号（含二次验证） | B 组全部 |
| **F 组验收** | 提供公司内部网关的 base_url / 模型名 / 鉴权风格，并配好 API 密钥 | F1/F2/F3 的真实验收 |

用户已明确表示**账号名稍后给**。在拿到之前，能做的是 C/D 组剩余项
（C2–C6、D3），但它们依赖 B4 校准后的解析器，所以实际上**大部分工作确实被卡住**。

**别去催，也别为了"有产出"而去做依赖图下游的事。** 计划的依赖图是真的。
...
| **增量** | `routes/delta.py` *(待建)* | **完全登出**，无 cookie | 每天 | **无账号可封** |
...
FacebookScraper/
  ...
  routes/
    backfill.py            登录态回填
    delta.py               ← 待建（C 组）登出增量
    fb_graph.py            API 只读通道，保留但未接入（缺 Token）
  publish/                 ← 待建（G 组）
  tests/                   9 套 282 项检查，全绿
...
**离线测试基线：9 套 282 项检查，全绿。** `scripts\setup.bat` 用
...
C 组剩余（C2–C6 登出增量）、D3（检查接入增量）、E 组（定时调度）、
...
- **`core/parse.py` 的解析器从未与真实响应比对过。** 这是最大的风险，
  但已有兜底（回填会在解析前无条件转储原始响应），不会造成数据损失。
...
- **远端**：`<首次 push 后把 URL 填在这里>`　**分支**：`main`
