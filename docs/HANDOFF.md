...
**代码与证据同步至 2026-09-26；服务机事实以各条观测日期为准。** 业务功能以 [FUNCTIONALITY.md](FUNCTIONALITY.md) 为准，每个验收单元的状态以 [REQUIREMENTS §10](REQUIREMENTS.md#10-五阶段验收状态) 为准；**本文件管的是边界与证据**——哪些是红线、真实 UI 长什么样、踩过什么坑、哪份证据能证明到哪一步。按主题组织，不记实施过程。
...
隔离证据位于本工作树 `state/offline-validation-20260922T121309Z/`（月历、生产回读、发布）、`state/offline-validation-20260922T121531Z/`（批准、单篇排期、静态检查）及 `state/offline-validation-20260922T121645Z/`（修正旧测试的提交函数文本匹配后，提交状态机、静态检查及 hygiene 通过）。原文本匹配改为实际提交桩读取 journal，证明点击前未决意图已经写入。

性能证据仅限隔离夹具：连续 10 次 G1 判断，单次独立作用域与共用同步作用域的截图物理路径核验为 20 次 / 2 次；一次采样耗时 53.6 / 46.0 ms。该数值不代表真实服务延迟。单渠道专属操作由 4 项减少为 2 项，执行入口能力检查由 2 次减少为 1 次。
...
两张表曾随设置页只读展示；2026-09-23 起设置页移除，改由 `GET /api/settings` 的 `controlled_fields` 提供。保存端点只接受默认时刻和挂起天数。付费入口严格解析：缺表当作空名单；名单里的空字符串、坏表、冻结名单超出品牌名单，或 `[targets]` 落在冻结名单上，都停止付费。监测告警用宽松解析，坏表当空集继续报。抓取入口只拦能读出的冻结目标；名单结构读不出来时不中止轮次。大小写和首尾空白在付费、告警和抓取入口都归一。
...
**已实现的取证基础与补验：离线通过。** 图片只读补验向回读传入原 attempt，由 `snapshots.load_for_attempt` 核实冻结正文、图片字节、来源指纹版本及绑定时刻。目标详情重开前后核对全文、账号、唯一渠道、时刻、remote ID 和网格稳定性；普通月份读取及正常提交回读不下载图片。`media.compare_ordered` 沿用 dHash ≤ 8、RGB 平均误差 ≤ 12、比例误差 ≤ 1% 的判据，保留相同字节的每个位置；坏图、缺图、多图、调序、意外重复及相似图歧义不能通过额外图片核验，不作为正常选图上传前置。
...
对[真实刷新结果](../state/g8-channel-refusal-20260924T033618Z-aa1145/calendar-refresh.json)做[离线规划回放](../state/g8-channel-refusal-20260924T033618Z-aa1145/planning-replay.json)，原时刻与另外两天均因独立时刻未核验而拒绝；`evaluate_slot` 将这些 `ProbeRequired` 统一映射为 `channels_unavailable`，末尾“请重新选择”不代表换时刻能解决。保留现有占用约束，不能借外层格子日期跳过未知变体。诊断与原始 API 返回按 [SHA-256 清单](../state/g8-channel-refusal-20260924T033618Z-aa1145/preservation.json)保全到主检出；没有重试提交或改变冻结内容。

**聚合 Post 的入口判定缺口。** 用户在服务机对 09-15 19:17 的 Post 运行只读 `--verify-reader`，[完整输出与摘要](../state/g8-aggregate-routing-20260924/operator-summary.json)确认请求对象 `122187915260939228` 的 `cross_posted_entities` 明确关联 FB 自身与 IG `18129143875786241`，各有独立 owner ID；37 个响应、预览已加载，仍以 `identity_unverified/channel`、零变体结束。原日志 SHA-256 为 `2dc341b9504f9c7422c6843d60a0c0cccf7d8f4b54a630fba289918a09095feb`。该错误路径说明入口走了无渠道的单视图读取，但原探针未保留表头/页签，不能据此确定真实控件的到达顺序或声称只是延迟。

`published_details` 现在在原预算内重读表头和平台页签，累计保留已观察到的徽标、渠道页签及已绑定本条 `content_id` 的原生成员所要求的渠道；空平台列表不证明单渠道，后续重绘不能撤销先前已经观察到的渠道。已知跨发项缺任一渠道页签或成员证据冲突时保持拒绝，读取结束再次核对渠道覆盖。各渠道仍走原有独立账号、ID、正文、时间校验，不把原生关联当作成功回执；未知内容类型保留原诊断。`--verify-reader` 复用既有脱敏 `SNAPSHOT`，增加 `DETAIL_STRUCTURE`，保留表头、平台标识、页签与加载状态，便于下一次实际复验；不保存正文或签名 URL。隔离用例先复现相同 `channel` 错误，以及只读 FB 就误判跨发完整的缺口；这些加载顺序是离线构造，不是原日志的 DOM 回放。服务机单条读取、完整月历与新排期仍须按实际结果验收，G8 完整图片适配状态不变。

**聚合入口修复验证：离线通过。** [验证汇总](../state/g8-aggregate-routing-20260924/verification.json)记录修前复现、共享读取器六脚本通过，以及最后的页签累计修订后聚合 8 项用例与 hygiene 复验通过；探针 15 项用例通过，覆盖新增结构输出与脱敏。独立复审发现的两项漏渠道问题均有先失败后通过的回归，最终无阻塞意见。接入 main `6949a10` 后，月历 API、发布操作与 hygiene 三脚本通过；这次集成未改变四个已验证的代码/测试文件。证据按 [SHA-256 清单](../state/g8-aggregate-routing-20260924/preservation.json)保全到主检出。各轮版本和范围在汇总中分列，不把它们称为最终版本的一次全量回归，也不替代真实服务机复验。

**聚合 Post 的后续真实读数与响应额度。** 服务机 [14:04 探针与最新操作摘要](../state/g8-aggregate-response-budget-20260924/operator-summary.json)已读出完整 FB 变体；临时详情的 Instagram 页签、徽标、7:18pm 表头与预览均就绪，但以 `aggregate_identity` 缺失结束，响应观测达到 `40/40`。原生产采集器的整页 40 条上限可在切换前被统计响应用尽；隔离浏览器按这个顺序复现相同错误。旧探针还漏记 `instagram_post`，所以不能从日志缺少该字段断言远端没有返回它，也不能确定超限后丢了几条。最新操作 `145e0cf752654d208fb2754d309744fc` 的请求时刻是 09-30 17:00，第 0/7 步失败，仍为同一冻结快照，`publication/schedule` 为空；未重提。03:53 的月历缓存仍显示旧 `channel` 错误，不当作这次提交的实时清单。

聚合 Post 现按初始加载、FB、IG 各保留 40 条被动响应采集额度，整页最多 120 条，重访不重置。切换前同步生产采集器和可选探针的视图；视图只分配额度，身份仍须由原对象、成员媒体 ID 和 owner ID 关联。原账号、正文、独立分钟及缺渠道拒绝规则保留。探针输出各视图采集/丢弃数量，并补齐 `instagram_post` 的脱敏身份结构。每响应 4 MB、5 秒读取限制保留，不增加主动请求、不增加人工配置；真实 Post 和当次月份的复验结果见下文，不据此升级 G8。

**响应额度修复：离线通过。** [七脚本验证汇总](../state/g8-aggregate-response-budget-20260924/verification.json)覆盖聚合 Post 12 项、探针 16 项、hygiene、月份读取、Feed/Reel 和两类 Story；新增用例先复现原拒绝，再验证迟到 IG 响应可读、错误媒体/owner 仍拒绝、探针与生产同相位及额度不可通过重访扩张。独立复审无阻塞意见。原始日志、截图、API 只读返回与测试日志按 [SHA-256 清单](../state/g8-aggregate-response-budget-20260924/preservation.json)保全到主检出。额度按响应到达时所在视图计数，单视图超过 40 条仍会拒绝继续采集；真实复验须同时看丢弃数、身份绑定和逐渠道结果，不能把提高总上限当成真实成功。

**09-15 聚合 Post 与当次月份读取：真实通过。** 服务机 `364c7db` 的[用户探针日志](../state/g8-snapshot-submission-20260924/received-probe.txt)返回 `complete=true`、`missing=[]`、退出码 0；FB `122187915260939228` / 19:17 与 IG `18129143875786241` / 19:18 各自完整，正文长度分别为 333、309。三个视图采集 8/23/13 条响应，均无丢弃。只读 API 的月份缓存时间为 2026-09-24T06:51:42Z，六项完整性均为 true、未核实数 0。范围仅限这个已发布 Post 和这次月份清单，不证明新提交或远端 scheduled 图片；原始返回和[诊断摘要](../state/g8-snapshot-submission-20260924/diagnosis.json)按 [SHA-256 清单](../state/g8-snapshot-submission-20260924/preservation.json)保全。

**另一稿件被旧版冻结来源指纹拒绝。** 本次为 `fa_neakasaofficial/122128135023379375`，不能套用上文的 Riko 稿件。操作 `f9723dd77dc6497290b31f0f835009a2` 的目标为北京时间 09-30 17:50，在第 0/7 步失败，快照为 09-22 冻结的 `cf2518f253e24db4885658bb75062dfa`。用户[本地只读诊断](../state/g8-snapshot-submission-20260924/operator-diagnostic.json)确认：v1，未绑定时刻，无本帖 attempt；正文、帖子、渠道和来源账号匹配，原冻结来源摘要有效，只有 media 改变，忽略已知签名后身份相同。根因是 v1 包含图片 URL 签名，符合 §1.31 的旧版兼容约定；不是正文或本地原图字节被改，也没有进入提交。旧快照不自动迁移，须由人解除冻结、复核图文并生成 v2 快照。该稿当前正文包含 `Hello......` 和 `This is a test for ...`，正式确认时须由人决定保留或编辑，不能代改。

**失配定位与提前核对：离线通过。** 提交校验按正文、帖子编号、渠道、来源账号、来源指纹和版本列出失败项，v1 来源失配补充图片链接提示。在访问月历前只读核验；月份读取后仍重新核验并只在通过后绑定时刻。旧指纹、防重、发布锁和追加式 journal 保持原判据。[七项定向验证](../state/offline-validation-20260924T070651Z/results.json)覆盖批准、人工单篇排期、操作状态、快照恢复、来源版本、原排期补验和 hygiene；含真实临时归档中的 v1 换签名拒绝、显式解除/重冻到 v2、旧文件保留，以及月历读取期间来源变化再次拒绝。[独立只读复审](../state/g8-snapshot-submission-20260924/review.json)无阻塞意见。 接入 main `4c37734` 的 Ctrl+C 修复后，[相同七脚本复验](../state/g8-snapshot-submission-20260924/integration-validation.json)通过，发布实现与复审版本一致。外部读取/提交用替身，没有真实排期；修复后的服务机提示与本帖重新冻结仍待用户操作。

**业务选图上传简化（2026-09-24）：离线通过。** 用户要求消除阻断正常发布的缩略图细节检查。服务机只读记录中，稿件 `122128135023379375` 已使用 v2 快照 `edee0ea6e27341c38bdc2c2cfbb394e9`；操作 `f63028f34b454925ba084ee76597da96`、attempt `aa2110ca-bd86-4f12-9d69-9a0352d7b41b` 在 G3 以 `failed_pre_submit` 结束，目标为北京时间 09-30 18:30，尚无提交点击或远端 ID。失败来自旧代码强制要求缩略图已加载且地址为 HTTPS fbcdn；旧记录没有保存失败地址类型，不能断言具体是 blob 或哪种域名。用户明确更正：右侧“加载中”图案属于图片内容，不是平台忙态。原始 API 返回、截图和更正见[现场摘要](../state/g8-composer-media-20260924T073654Z/diagnosis.json)。

正常发布移除缩略图域名、二次下载、视觉相似度和比例门槛，删除与实际上传完成无关的整页 networkidle 等待；按冻结文件顺序上传，只核对附件数量和平台上传提示。同一组预览附件确实调序时仍停止，地址转换、缺缩略图或预览像素不作失败依据；地址只取内存摘要，编辑器记录明确 `order_verified=false`。最后先等排期按钮可见且启用，再复核表单并落提交意图；持续禁用属于提交前失败。远端图片比较与 G8 证据独立保留，不宣称新布局或真实提交通过。

正常提交回读不再附带第二次开图和媒体采集；月历已经确认的唯一新排期按身份、正文和时刻记录 `scheduled`，图片未核验单列。`records.reverify_media` 显式启用图片读取，继续绑定原 attempt / remote ID 追加证据；正常路径即使额外取图入口会失败也不触发它，远端编号变化仍保持未决并防重。[相关五脚本验证](../state/offline-validation-20260924T075640Z/results.json)覆盖普通回读、只读补验、工作流和 hygiene。

本次检查覆盖从冻结到回读的主流程：来源许可、冻结字节、账号/唯一渠道、选定时刻与同渠道占用、发布锁、防重、点击后不重试均保留；原有 v1/v2 签名兼容保持原约定；单篇提交已与历史控件验收及 G8 解耦，不新增人工配置。[首轮三个定向脚本](../state/offline-validation-20260924T074552Z/results.json)通过，涵盖本次失效形态及按钮就绪前不写未决意图；最终 13 个相关脚本均通过，外部账号、素材和排期均未操作。[验证汇总](../state/g8-composer-media-20260924T073654Z/verification.json)保留各轮失败与修复结果；原始现场、日志及汇总按 [SHA-256 清单](../state/g8-composer-media-20260924T073654Z/preservation.json)保全到主检出。[独立只读复审](../state/g8-composer-media-20260924T073654Z/review.json)未发现阻塞项。真实恢复排期仍待用户明确确认当前稿件后执行。

**日期显示格式误报（2026-09-24）：离线通过。** 服务机 attempt `43edd106-ec9b-4a01-bdf9-d4af507bbb6f`、操作 `3ae3de6f9c17447a9fffeed5875228f3` 已通过附件检查，在最终表单复核以 `failed_pre_submit` 结束，无提交信号和远端编号。目标与 UI 时区均为 `2026-09-30 23:00 / Asia/Shanghai`，原填写回执为 `9/30/2026 11:00 PM`，用户截图显示 `Sep 30, 2026 11:00 PM`。旧解析器只识别数字斜杠日期；在实际工作流的隔离 Chromium 夹具里，仅替换同一天的显示格式即可复现同一报错。原始 API、四张截图及[诊断摘要](../state/g8-schedule-form-20260924T081858Z/diagnosis.json)保留；未读取真实页面 DOM 或执行提交。

填写后和提交前共用日期、时间语义核对：日期支持既有美式输入、英文月份及 ISO 值；时间支持 input 值和可编辑字段文字，不强求容器再重复时间，允许补零/空白/大小写等同值变化。填写日期和最后时间字段后 Tab 失焦，再读实际值；日期确实回退或变化仍停止。错误分清未知日期与实际不符，附确认值、时区和页面值。隔离浏览器覆盖原样 09-30 23:00、中午/午夜、跨日/月/年、控件失焦回退及未知日期；已有时区偏移、DST、发布锁和单次提交约束保持。最终六脚本验证、独立复审与保全见[本次汇总](../state/g8-schedule-form-20260924T081858Z/verification-final.json)，真实排期仍待服务机更新后按当前冻结内容确认。

**9223 窗口退出与读取负担（2026-09-24）：待真实联调。** 服务机 `ce71a89` / Playwright 1.62.0 的 attempt `2f4f1d45-e53f-4d1f-a360-99f9cc77d836` 停在“附着发布 Chrome”，为 `failed_pre_submit`，没有提交信号或远端编号。用户报告整窗消失；事后监听已恢复（PID 24880），Windows Application 1000/1001 和 System 2004 查询没有输出，物理内存剩余约 5.1 GiB。这些不能锁定故障瞬间的 Chrome 退出原因，也不能证明 OOM、平台封禁或依赖版本缺陷。原 API、截图、用户诊断与离线复现见[诊断记录](../state/g8-browser-stability-20260924T091704Z/diagnosis.json)。

已复现的代码问题是单篇排期反复展开整月历史详情、明确提交前失败后留下编辑器/月历页、同页可并发读取 40 个响应正文。单篇改为完整网格加相关完整日期详情；范围外日期不授予裁决能力，整月同步与删除核实保持。自建页在成功或提交前失败后释放，仅准备/未决编辑器及人工页保留，最后一页转为空白页保留 Chrome。连接只断开；详情串行且释放后间隔 1 秒，响应正文并发最多 4、结束时清理剩余任务。双平台 Story 包裹的页面关闭/崩溃异常也必须停止整个读取，不能降为普通身份缺口继续访问。

本机 Playwright 1.62 与 1.63 的隔离 CDP 生命周期检查均通过：重复附着/断开保留人工页，清理最后自建页后仍可连接。这里使用无登录临时 profile 和本机 Chrome，不能代替服务机真实账号验收；没有要求升级 Playwright。[1.62 日志](../state/g8-browser-stability-20260924T091704Z/playwright162-lifecycle.log)、[21 个相关脚本的最新通过结果](../state/g8-browser-stability-20260924T091704Z/verification.json)及[独立评审](../state/g8-browser-stability-20260924T091704Z/review.json)保全；中间轮次的清理诊断失败由最后六脚本通过替代，仍缺更新后服务机运行结果。操作顺序见 [MANUAL_STEPS §16.3.1](MANUAL_STEPS.md#1631-发布-chrome-窗口消失或-9223-断开)，未执行新真实排期，不升级 G8。

**服务机离线表单测试启动失败（2026-09-25）：离线通过，服务机三脚本复验已通过。** 用户补充的 `offline-validation-20260925T104935Z/tests_final_form.log` 确认 13 个用例在 `asyncSetUp` 找不到 `chromium_headless_shell-1234`，未执行表单断言；初始化失败跳过原 `asyncTearDown`，遗留 Playwright 驱动随后触发 Windows 管道清理异常。测试现与生命周期夹具一样使用既有 Chrome 程序的无登录临时会话，每取得一项资源立即登记异步清理；启动、上下文、页面初始化失败均清理已取得资源。离线入口显示首尾错误和完整日志路径，不再只显示最后 3500 字符。本机空浏览器缓存下[五脚本验证](../state/offline-validation-20260925T105504Z/results.json)通过，缺失 Chrome 路径的真实驱动故障注入保留原失败且无退出清理异常，见[诊断与证据](../state/g8-final-form-startup-20260925/diagnosis.json)。此修复仅涉及测试与诊断，不改变生产浏览器、表单校验或 G8 状态，不能用本次测试错误解释前一天真实 9223 整窗退出。 用户提供 `offline-validation-20260925T110638Z` 的三脚本均通过，仍只是离线验证。

**G5 显示时间正确但字段为空（2026-09-25）：离线通过，修复后真实排期待联调。** 操作 `3d9a754775384f33a77606340cf9194d` / attempt `a22fce76-359f-48c2-b245-e68ccdac0574` 在 G5 失败，无提交信号和远端编号；目标为 09-30 23:00 / Asia/Shanghai，容器回读为 `11 : 00 PM`，三个 input 值均为空。旧 `_read_time` 强制要求 input 拼出完整时刻，拒绝了已录证的容器显示。只读当前控件探针返回空列表，不能断言 Meta 内部如何清空字段；合成失焦清空编辑值的真实浏览器场景复现了逐字相同的错误。现在仅在三个字段全空且同一容器全文匹配唯一完整时刻时，使用该当前显示值；仍有字段值、部分缺值、冲突、额外文字、多个时刻或实际改期均不靠此规则放行。填写后与提交前共用读取，不用预期值填补观测。新增工作流用例实际执行 G5 与最终复核，提交用替身，覆盖同值单次提交及等待期间改期/字段冲突在提交意图前停止。原 API、用户读数、红绿回归与限制见[诊断记录](../state/g8-time-readback-20260925/diagnosis.json)；[五项定向脚本](../state/offline-validation-20260925T112205Z/results.json)及 Playwright 1.62 空缓存下的 18 项表单测试通过，独立复审无阻塞。没有新的真实排期或 G8 升级。
...
服务机待提供每渠道一个原 scheduled attempt、绑定快照与有效渠道资产记录，以及同一 remote ID 的详情结构、独立总数/末项、各位置实际字节与轮播加载关系；优先补验已有多图对象。没有合格对象才按 [MANUAL_STEPS §16.3–§16.4](MANUAL_STEPS.md#163-走一遍完整的人工流程) 展示具体冻结内容并取得确认，不能为补证重复排期。本分支已接入复查时最新 main `86f0212`，保留其 UI/历史筛选、列表占位、月历来源关联及确认预览改动；本次真实样本确认仍需另展示柏林时刻，不恢复主干已撤下的前端时区显示。

**定向验证与复审：离线通过。** [既有发布子系统 16/16](../state/offline-validation-20260923T090122Z/results.json)覆盖批准、单篇排期、发布操作、最终表单、渠道/月历、占用、缓存/API、冻结恢复、图片选择和来源指纹；接入新主干后的 [4/4](../state/offline-validation-20260923T090651Z/results.json)复验恢复、补验、工作流/验收链和 hygiene。加上新增取图/浏览器/比较脚本，逐脚本最近结果共 21 项通过；这是多轮定向结果，不是一次全库回归。英文提交使用现有 Git 身份，无 AI 署名；配置不变，19 个改动文件 UTF-8/LF 与新增文档链接已核对。G1 原 JSON 摘要与 §1.21 相同，379 张截图保留。日志及文件检查见验证汇总，同名路径日志已复制到主检出；[逐份保全清单](../state/g8-remote-images-20260923/preservation.json)记录来源、去向及 SHA，原件仍留本树。

独立只读评审覆盖 `3bb04f3..ae66f24` 的 19 个文件及调用方，没有需修改项；复核的 21 份日志摘要均一致。范围裁决如下：

| 评审保留项 | 本轮裁决 |
|---|---|
| FB/IG 完整列表、轮播与真实压缩/裁切判据 | 保留代码缺口及真实样本要求，不猜布局或放宽阈值 |
...
**同日多帖的详情读取与排期通知（2026-09-26）：待真实联调。** 服务机只读输出确认 `122128135023379375` 的 `2026-09-30 23:00 +08:00` 原 attempt `57da3ab1-81a8-41db-90c3-5640edcdb0e5` 已为 `scheduled`、远端编号 `facebook=1084557747316275`；新 `122125865115379375` 的同日 `17:30` attempt `2d85e661-8bb9-429a-a1e6-ce6cf9686d67` 仍是 `submitted_unverified`。新核对的唯一诊断来自第 2 张 `11:00 PM` 卡，`stage=scheduled_detail / code=read_failed`；排期通知已启用，旧成功事件存在但 `deliveries=[]`，即尚未尝试发送。[服务机输出](../state/g8-same-day-notifications-20260926/service-diagnosis.json)由用户消息转存，本机只读 HTTP 连接未成功。

同日双卡本身可以区分；真正连带阻塞发生在正向回执复用了 `cards_in_range` 的空档规则：另一条详情失败便丢失独立时刻证据，导致已经完整读到的目标也不能确认。现只有 `verify()` 使用正向对象匹配，仍要求网格稳定、目标自身时间/账号/渠道/正文/编号完整、已观察到的匹配恰好一个，并保留其它卡片的诊断。它不推断未知聚合成员的真实时刻，不证明其它对象不存在；提交前基线、间隔规划和删除登记的完整性规则不变。23:00 详情本身为何报 `read_failed` 尚未取得底层异常，不能称为已修复；本轮修复的是它错误阻断另一目标回执的链路。
...
定向后端、前端测试与构建通过，通知/正文就绪验证见 [verification.json](../state/g8-same-day-notifications-20260926/verification.json)，按服务机诊断复现的回执规则修复见 [receipt-verification.json](../state/g8-same-day-notifications-20260926/receipt-verification.json)。全部为隔离数据、合成页面或模拟机器人；未发送真实群消息、未重新排期。服务机更新后须沿上述两个原 attempt 补回执/通知，真实送达和新帖恢复仍待核对，不关闭 G8。
...
`[publish].require_all_media_de = true` 要求有效德语图或对当前原图的明确确认。没有需要本地化内容的图片可由审校人确认原图；像素变化率仍只是辅助提示，不能代替这项人工决定。
