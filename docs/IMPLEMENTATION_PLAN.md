...
- [ ] **C2** 实现 Instagram 登录态增量
  ...
  >
  > 【离线已验证】`tests/tests_delta_logged_in.py` 82 项：登录墙四种长相、
  > 深度上限、幂等、归属过滤、残缺帖升级、dry-run 不写盘、四类异常即停。
  > 【真实验收待办】owner 全对、连续两次第二次 0 新增、`--dry-run` 不写盘。
...
- [ ] **C3** 实现 Facebook 登录态增量
  ...
  > 进行中：2026-08-30 · 与 C2 共用 `delta_once()`，平台差异只在
  > `profile_url()` 与 `core/parse.py` 已有的解析器分派。`partition_by_owner()`
  > 照常调用（离线测试里有一条 FB 归属断言：取的是 `actors[0].url` 的账号名段，
  > 不是展示名）。同样等真实验收，不勾选。
  >
  > ⚠️ **业务上 FB 才是增量真正有价值的那半边**：2026-08-30 核对归档实测，
  > FB 发帖中位间隔 **1.0 天**、最新一帖 2026-08-25（距今 4 天），
  > 而 IG 最新一帖 2026-07-15（距今 45 天）。归档里**大概率已经缺了
  > 8-25 之后的几篇 FB 帖**——这正好是 C3 首次实测的天然靶子：
  > 如果一篇都没抓到，那不是"没新帖"，是有问题。
- [ ] **C4** 增量的媒体下载
...
- [ ] **C6** 增量主入口
  ...
  > 进行中：2026-08-30 · `python -m routes.delta`，参数：
  > `--platform` / `--dry-run` / `--if-stale` / `--no-jitter` /
