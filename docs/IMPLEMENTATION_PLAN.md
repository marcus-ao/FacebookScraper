...
FacebookScraper/
  ...
  tests/                   离线测试。scripts\setup.bat 用 glob 全跑，新增即自动纳入基线
    ...
    tests_translate.py       翻译管道 + 提示词渲染 + 金额保留强制（162，零 API 调用）
                             ——合计 10 套 400 项，**在 stdout 被重定向的条件下也全绿**
...
- [ ] **C2** 实现 Instagram 登录态增量
  ...
  > 本项原为「Instagram 登出增量」，2026-08-30 随方案 B 重写。
...
>
> ⚠️ **`[integrity].alert_after_quiet_days = 4` 必须重设。** 它是按"日均约 1 帖"
> 定的，但实测该账号**连续一个多月没发新帖**（IG 最新一帖 2026-07-15）。
> 不改的话每个月都会误报，而误报多了真报警就没人看了。
> 建议先按实际发帖间隔统计一遍再定值。
