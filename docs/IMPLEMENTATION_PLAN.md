...
FacebookScraper/
  ...
  tests/                   离线测试。scripts\setup.bat 用 glob 全跑，新增即自动纳入基线
    ...
    tests_delta_logged_in.py 登录态增量：异常即停、深度上限、状态记录、
                             失败预算、抖动顺序、D3 接入、合作帖（103）
    tests_schedule.py        计划任务 XML + .bat 字节级约定（47）
    ...
    tests_translate.py       翻译管道 + 提示词渲染 + 金额保留强制（162，零 API 调用）
                             ——合计 12 套 578 项，**在 stdout 被重定向的条件下也全绿**
...
- [x] **E1** 编写 `scripts\run_delta.bat`
  ...
  > 2. **只有不带参数时才 `pause`**（即双击进来的情况）。计划任务永远带
  >    `--if-stale`，所以它不会卡在 pause 上等到超时。
...
- [ ] **E3** 注册 Windows 计划任务
  ...
  > `install` 支持 `--dry-run`）。47 项离线断言在 `tests/tests_schedule.py`。
  >
  ...
  > | 任务名 | 触发器 | 参数 |
  > |---|---|---|
  > | `FBScraperDelta` | 每天 `[delta].daily_time`（默认 09:30） | **无** |
