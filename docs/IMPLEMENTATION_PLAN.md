...
- [x] **K1** `Settings` 与 `--check` 自检

  - 照 `translate.py::Settings` 写，`validate()` 在任何网络请求前给出可操作错误
  - `--check` 发一个最小真实 edits 请求，并断言：网关可达 / 鉴权通过 /
    `/models` 精确存在 `gpt-image-2` / 显式响应 model 不错配 /
    `data[0].b64_json` 能解码成合法图片 / 最低 `usage` 契约齐全
  - ⚠️ **`--check` 会真的花钱**，输出里必须说清楚（与 F 组口径一致）
  - 【验收】真实 Key 下通过并打印实际 model 与 usage；缺 Key 时给出复制
    `.env.example` 的完整命令且**不联网**
  > 完成：2026-09-01 · 每客户端先做一次 `/models` 精确预检；响应 `model` 缺失时以
  > 请求模型 + 目录验证记为 `catalog`，明确错配仍熔断。`output_tokens_details` 改为可选，
  > 输出费用取顶层 `output_tokens`，超时提高至 720 秒；预检失败同样缓存并立即停批，
  > SDK 对已付费 2xx 响应解析失败也列为致命。真实 low 自检返回 816×816
  > 可解码图片，model 验证为 `catalog`，usage input/output/total=1043/171/1214。
...
- [ ] **K7** `--estimate` 与费用汇总
  ...
  > **进展：2026-09-01** · 已有一张 high 真实样本，`--estimate` 算出上游
  > US$0.2546/张并打印 dHash=9；尚未与 Inferera 控制台账单比对，故仍不勾选。
- [ ] **K8** 接入审校清单　⬆️ **K3 取消后优先级提高，它是 K9 的唯一依据**
  ...
  > 确认六个 Markdown 引用可显示。
  > **进展：2026-09-01** · 已有一张真实 GPT-Image-2 产出，目标帖在 `review.md`
  > 原图/德语图并排显示；仍只有 1 篇且未由懂德语的人勾选，不能冒充三篇验收。
- [ ] **K9** 端到端真实验收
  ...
  >
  > **单图技术进展：2026-09-01。** 用户限定只做一张后，帖子
  > `3975547640610092585` 的 high 调用成功：1350×1687 → 1344×1680，157.891 秒，
  > 0 失败，dHash=9，usage input/output/total=2811/7869/10680，上游估算 US$0.2546。
  > 原图与 manifest 哈希未变，产出哈希与 `images_de.jsonl` 一致；原命令复跑命中跳过，
  > 0 API、0 重复写盘。剩余 10 张未执行，懂德语的人也未逐图确认，故 K9 仍不勾选。
  >
  > **进展：2026-09-01（G 组会话记录）· 用户已确认那一张图「符合要求、完全可用」。**
  > 也就是说 K9 的【验收】在**这一张**上是过了的：文字正确、
  > `IMAGE_PLAN` 3.1 那些不可改项没被动过、产品外观没变。
  > ⛔ **仍然不勾选**，因为【验收】的范围是"最新 3 篇的**全部**配图"，
  > 而 11 张里只跑了 1 张 —— 剩余 10 张需要用户单独授权费用。
  > **勾选条件因此收敛成一件事：补跑剩下 10 张并逐图确认。**
  >
  > 顺带一条对 G 组有意义的事实：这一张让 **K→G 的所有权契约第一次在真实数据上
  > 双向成立** —— `tools/compose_publish.py` 对该帖打出的图源是「德语图」
  > 而不是回退原图，此前只有夹具验证过。
...
- [ ] **G1** 探查 Business Suite 的真实 DOM 与流程　**← 需要你操作，且它阻塞 G2–G7**
  ...
  > TODO，故不勾选。
  >
  > 进展：2026-09-01 · **用户已录到两份真实 dump，【验收】那一条已经满足，
  > 【记录】那一条还没有——所以按工作协议仍不勾选。**
  >
  > | | 状态 |
  > |---|---|
  > | 【验收】`selectors.py` ≥7 条带注释定位 | ✅ **12 条**（全部 composer 界面），每条写清「对应哪一步 / 出自哪份 dump 的第几条 / 什么信号说明它失效」 |
  > | 【记录】定时窗口上下限 + 时区行为 | ⛔ **没有**。`observations` 整段是空的（那一轮被 CR-66 的死锁中断，收尾问答没跑完） |
  >
  > 两份 dump：`publish_probe_20260831_222850_347719.json`（**24 条 ·
  > `business.facebook.com/latest/composer/`**，回填就靠它）与
  > `publish_probe_20260831_195237_008047.json`（63 条 ·
  > `www.facebook.com/professional_dashboard/`，**另一个界面，只作旁证**）。
  >
  > ⚠️ 那份 24 条 `finished_at=null` 且序号不连续，**过不了
  > `compose._validated_probe_dump` 的严格校验，这是对的**（那两条校验证明的是
  > "记录没丢过"）。但**人工读来回填定位完全够用**。
  > ⛔ 没有手工改序号去"修好"它。
  >
  > **回填方式**：`publish/selectors.py` 现在是一张**带证据的登记表**
  > （`Locator` / `Gap` 两个 dataclass），不是一堆裸字符串。
  > 新增 `publish/evidence.py` + `tests_publish.py` 第 [5] 节：
  > **逐条把定位回查到它自称的 dump**（role + 可访问名 + 稳定属性都要对上），
  > 本机 14/14 全部命中。**红线 5 从此是一条可执行的检查，不再只是纪律。**
  > dump 不进版本库，所以别的机器上是"跳过"而不是"通过"，两者分开打印。
  >
  > **G1 没录到的东西写在 `selectors.GAPS` 里**（每条写清
  > 「为什么没有 / 挡住了谁 / 怎么补上」）。第四轮生产缺口已经收敛为同一份
  > 完整 v2 证据链：目标账号上下文、可信提交交互、被动成功语义、Planner 数据
  > 就绪、同一卡片内独立的时刻/正文/FB/IG/图片语义与有效 final；排期窗口和 IG
  > 上限也必须写入实测观察项。`--submit` 强制严格约束。FB/IG 勾选控件仍不录
  > 不点，因为 composer 默认全选，提交后由结构化渠道子语义回读兜底。
  > 另外三条（composer 的 file input、小时 spinbutton、缩略图容器）改用
  > **运行时发现 + 回读校验**绕过，不补录也能走，详见 G3/G5 的完成行。
- [ ] **G2** 登录态与**目标 Page**检查
  ...
  > 进展：2026-09-01 · 手工“准备到提交前”仍只能做页面文本的弱核对；
  > **生产 `--submit` 已升级成强证据闸**。
  > `ensure_logged_in()` 不再返回 `bool`，返回 `AccountContext`——
  > 因为"登录了没有"和"选中的是不是对的 Page"是两件事，把它们压成一个 bool
  > 就等于把"没查出问题"和"查过了没问题"混为一谈。
  >
  > `ensure_logged_in()` 返回 `AccountContext`，区分“有登录态”与“选择已验证”。
  > 默认准备可以保留弱核对并要求人工确认；显式 `--submit` 则必须先从配置审核的
  > 同一份 v2 dump 回查 `composer_account_context`，其中 FB Page 与 IG account token
  > 必须逐字对应当前配置，否则在上传和点击之前失败闭合。真实 DOM 尚未验收，故不勾选。
- [ ] **G3** 图片上传
  ...
  > 进展：2026-09-01 · 已实现，走的是 **Playwright 的 file chooser 通道**
  > （点录到的 `Add photo/video` → 截住浏览器弹出的文件选择器 → 交文件），
  > **不写任何 `input[type=file]` 选择器**。三个好处：
  > ① 连 HTML 标准控件都不用碰；
  > ② **当场回答 G1 的第 2 问**——弹得出 chooser 就是文件输入，弹不出就是拖拽区，
  > 两种情况报错文案不同；
  > ③ `chooser.is_multiple()` 是一条真回读：只收单文件却要传 5 张时当场失败，
  > 而不是默默只传一张。
  > 旧版后台那份 dump 的第 33/34 条（`input type=file` / `multiple=true` /
  > `accept=image/*,…`）是"背后确实是真文件输入"的旁证，已原样收进
  > `selectors.dashboard_file_input`。
  >
  > ⛔ **【验收】不勾**：缩略图容器没进 dump（`GAPS.composer_upload_thumbnails`），
  > 程序能确认"文件已交给控件"，**不能**确认 UI 收下了几张。
  > 这一条以返回提示的形式交回给人复核，**没有假装验过**。
- [ ] **G4** 文案填写
  ...
  > 进展：2026-09-01 · 已实现：正文框确认是 **contenteditable**
  > （dump 第 5 条命中的是内层 div，第 3 层语义祖先才是 `role=combobox`），
  > 所以**逐行 type + Shift+Enter**，不用 `fill()`。
  > 回读走 `inner_text()` 并只做两项归一化：去零宽空格（dump 第 10 条录到的
  > 输入值就是 `"He​"`）、统一换行——**再多归一化就等于把这道闸关掉**。
  >
  > 输入 `#` 会触发话题标签自动补全（dump 第 7 条那个菜单）。**没有去躲它**，
  > 躲它就得猜菜单的定位；改成让回读闸去抓：补全真改了正文就当场失配并停下，
  > 报错里指出第一处不同。测试里模拟了一次补全改写，确认拦得住。
  >
  > ⛔ 【验收】只在仿真 page 上过（含换行/空行/emoji/äöüß/#标签/$金额），
  > **真实 UI 上一次都没跑过**，不勾选。
- [ ] **G5** 定时设置
  ...
  > 进展：2026-09-01 · 已实现，签名多了一个**必填**的 `ui_timezone`——
  > 时区不许从 `[publish].timezone` 隐式借用，也不许用本机时区。
  > 新增 `[publish].ui_timezone`（**留空 = 还没测 → 失败闭合**，报错里给出
  > `--fill-notes` 的原话命令）。严格模式下它还必须与 dump 里那条人工观察对得上。
  >
  > 三个控件都来自 dump：`role=switch` 的 `Set date and time`（第 22/23 条）、
  > `role=textbox` + `placeholder=mm/dd/yyyy` 的 `Date picker`（第 26 条）、
  > `role=application` 的 `Time input`（第 25/28 条，渲染成 `12 : 30 AM`
  > ——**这就是 12 小时制 + AM/PM 的直接证据**）。
  > **小时那个 spinbutton 没被录到**（用户只点了 minutes 与 meridiem），
  > 所以按**排除法**定位：容器里三个 spinbutton，两个有已知 aria-label，
  > 剩下那个就是小时；**个数不是 3 就失败闭合，不去猜哪个是小时**。
  >
  > 回读按语义比，不按字符串比（`12 : 30 AM` 的补零形态没被实测过）：
  > 日期用正则取 m/d/Y 与目标比，时刻取 `h : mm AM/PM` 与目标比。
  > 日期对不上时报错里点名"**先怀疑日期格式变了**"——格式变了而代码照旧
  > `strftime`，会**静默地排到另一个日子**，那是这一步最贵的错法。
  >
  > 【验收】的三种情况都在仿真 page 上跑过并通过：跨日跨月
  > （UTC 09-30 23:30 → 柏林 10-01 01:30 AM）、
  > **夏令时开始日**（03-29 01:30Z → 03:30，不是 02:30）、
  > **夏令时结束日**（10-25 01:30Z → 02:30，偏移已退回 +1）。
  > ⛔ **仍不勾选**：真实 UI 上没跑过，而 UI 时区这个输入本身还没实测。
- [x] **G6** 提交与结果确认（代码与离线验收完成；真实证据/G8 待用户）

  - `submit(page) -> SubmitResult`，必须等**明确的成功信号**，不能提交完就返回
  - 成功后写 `state/published.jsonl`（见 G6b）
  - 【验收】排期列表里能看到该帖

  > 实现：2026-09-01 · probe 已升级为 v2；`submit()` 只有一个 click 调用，
  > 点击异常或成功信号超时一律返回模糊状态，绝不重试。单帖入口默认停在提交前，
  > 只有显式 `--submit` 才进入 G6，而且该参数会强制启用经 probe 审核的严格 UI
  > 约束。现有 dump 仍缺完整的账号→提交→成功→Planner→final 证据链，所以生产
  > 预检会在碰浏览器前抛 `ProbeRequired`；这是证据闸，不是未实现。

  **生产启用按这个顺序做**：

  1. 用户重录探查：提交前录到正确 FB/IG 账号上下文；提交后录到成功语义，
     进入 Planner 等数据就绪，并让同一卡片的独立子语义完整显示时刻、正文、
     FB、IG 与图片数；保持 final 不动后停止，补齐实测 UI 观察值；
  2. 把账号、提交、成功、Planner 就绪与结构化卡片证据回填进登记表；
     `evidence.py` 会校验 v2 契约、同页顺序、final 和遮罩截图；
  3. evidence 与严格 UI 约束全部通过后才允许 `--submit`；
  4. 成功信号确认之后继续 G6c，未回读绝不写 `scheduled`。

  ⚠️ **这一项的实现顺序不许颠倒。** 先写点击、想着"成功信号以后再补"，
  等于先造出一个会真的往外发帖、却判断不了发没发出去的东西。
- [x] **G6c** 提交后回读：从"点过了"升级成"看得见它排在那儿"（代码与离线验收完成）

  > **G6 的成功信号回答的是"这一次提交成功了没有"，G6c 回答的是
  > "它现在确实在排期列表里"。** 两者不能互相代替：
  > toast 一闪而过，而排期列表是可以复查的状态。

  - 提交后回到排期列表/内容日历，先等待 probe 证明的数据就绪信号，再按
    `scheduled_at` + 完整文案指纹 + 目标 FB/IG + 图片数**找到这一篇**
  - 所有字段必须来自同一张卡片中相互独立的子语义；正文里碰巧出现日期、
    “Facebook”“Instagram”或“5 photos”不能当作元数据命中
  - 找到了才写 `journal.STATUS_SCHEDULED`；找不到就保持 `submitted_unverified` 并保留截图
  - ⛔ **`prepared` 不许自动升级成 `scheduled`**（`journal.py` 的核心不变式）：
    `prepared` 意味着 Business Suite 里**可能留着一份草稿**，
    把它当成"发出去了"，下一次就会静默地少发一篇或多留一份草稿
  - 【验收】连跑两次，第二次全部跳过、**零浏览器操作**（这条是 G6b 欠着的那一半）
  - 【前置】G6；排期列表项的定位同样要来自真实 dump
- [x] **G6b** 幂等与留痕 `state/published.jsonl`（五态追加式 journal）
  ...
  > 进展：2026-09-01 · `publish/journal.py` 已实现，只追加不重写。
  > **关键设计：`prepared` 与 `scheduled` 是两个状态，不许合并。**
  > 现在绝大多数记录是 `prepared`（填到提交前），而
  > **`prepared` 不构成幂等跳过的理由**——它意味着 Business Suite 里可能留着
  > 一份草稿，重跑会再留一份，所以入口会停下来让人先去看一眼（退出码 3）。
  > 只有 `scheduled` 才跳过。
  >
  > 人在浏览器里点完提交之后，用 `--mark-scheduled` 把上一条 `prepared`
  > **原样结转**成 `scheduled`（⛔ 不重新组装：中间若有人改过译文，
  > 记下来的就不是实际发出去的那一版了），留痕里写明这是**人工确认**、
  > 不是程序看到成功信号。
  > ⏳ 【真实验收】"连跑两次第二次全部跳过"只有逻辑层测过，真机部分归 G8，
  > 不影响本项的实现完成状态。
  >
  > 第四轮补充：状态现为 `prepared` / `submit_ambiguous` /
  > `submitted_unverified` / `scheduled` / `failed_pre_submit`。每次尝试写 attempt_id、
  > 全部 source_refs、渠道、正文/图片指纹、目标/UI 时刻、成功信号、远端标识与截图；
  > 兼容旧行。模糊/未回读以及未人工闭合的 pre-submit 失败均阻止自动重试；
  > 人工结转明确写 `manual_evidence=true`。只有最终 `scheduled` 算已发布。
- [ ] **G7** 失败处理与残留清理
  ...
  > 进展：2026-09-01 · `capture_failure()` 已实现：截图存
  > `state/publish_failures/<时间戳>_<步骤>.png`，打印步骤 + 当前 URL + 原因 +
  > 「程序**没有**删除任何东西，请自己去看有没有草稿残留」。
  > 截图**遮罩凭据类输入**，遮罩插不进去就不截——宁可没有截图，
  > 也不能把凭据拍进去（与 G1 recorder 同一条边界，有断言钉住两处选择器逐字相同）。
  > ⛔ 【验收】要在真实 UI 上人为改坏一个定位再看截图，没做，不勾选。
- [~] **G8** 端到端发布测试　—— 🟡 **2026-09-01 真机跑了六次，最后一次提交成功**

  - 前五次撞出 **CR-70 ~ CR-74**，全部是"离线假页面比真实 SPA 简单太多"；
  - 第六次**走到提交并成功**（成功信号 + 两个渠道时刻都对，
    Business Suite 上确实出现了那两条排期）；
  - 但 G6c 内容日历回读没证明它 → 留痕停在 `submitted_unverified`（**CR-75**，已修待验）；
  - ⬜ **验收条件仍未达成**：`state/published.jsonl` 里还没有 `status=scheduled`。
    重跑一次即可，操作见 `MANUAL_STEPS.md` 第 12 步。
...
- [x] **G9** 与每日增量的挂钩（assisted 代码完成；G8 后才激活）
  ...
  > 第四轮实现：`pipeline activate/run/approve/status` 已接入。激活边界原子写入且
  > 不移动；manual 只对账，assisted 推进 delta/按 ID 翻译/逐图处理并生成
  > needs_human，运行阶段不触碰发布浏览器。跨平台严格相同自动合并，0.90 以上相似
  > 或素材一致但版本不同先人工选版；2026-08-27 FB/IG 真实 5 图对的 dHash 均为 0，
  > 因 CTA 不同已由测试钉为待确认。approve 重新跑离线硬闸、读取远端占用槽，
  > 按柏林每天 10:00/17:00 分配，逐篇提交并在首个浏览器/提交模糊失败时停批。
  > `supervised`/`autonomous` 明确失败闭合。当前未 activate、配置仍为 manual、
  > 计划任务未安装；真实验收归 G8 之后的用户步骤。
  > `run`/`approve` 由流水线操作锁互斥；单帖发布另有发布锁，并在锁内重查 journal，
  > 防止两个入口重复提交。预算按所有账号的真实 usage 全局核算，损坏/未知账目失败
  > 闭合。`approve` 在读取远端槽位前后重做候选对账、作者/金额/离线硬闸、source_refs
  > 冲突与最终正文/图片指纹校验，任何陈旧或重叠确认项都不能进入浏览器。

  > **以下是实现前的历史任务书，已由上面的第四轮实现状态覆盖。** G9 是把前面
  > 四段串起来的线，不是新的业务真相源：
  >
  > ```
  > routes/delta.py 抓到新帖        ✅ 通（但计划任务仍未安装 = L0a）
  >   → translate.py                ✅ 通（--post-id 可精确选帖）
  >   → localize_images.py          ✅ 通（1 张已人工确认可用；其余需单独授权）
  >   → publish/compose.py          ✅ 通（离线硬闸）
  >   → tools/publish_post.py       ✅ 通（默认提交前；显式 --submit 进入 G6）
  >   → 【提交 + 成功信号】          ✅ 代码完成；生产等 v2 证据
  >   → Planner 回读 scheduled       ✅ 代码完成；真机验收归 G8
  > ```
  >
  > **所以 G9 的真实工作量不是"实现流水线"，而是三件小事**：
  >
  > 1. **一个只负责编排的入口**（`pipeline.py` 的 `run`，属 L 组 L1a）——
  >    ⛔ **它不许自己干活**：不解析响应、不拼提示词、不点浏览器，
  >    只调各段的入口、读各段的产物（`PIPELINE_PLAN` 第 2 节的边界）；
  > 2. **待确认清单落盘**，让人可以攒着一次看完，而不是每篇都被拦一次；
  > 3. **`[pipeline].autonomy` 真的被读**（第四轮已接通）：
  >    `manual` / `assisted` / `supervised` / `autonomous` 决定哪几段自动、
  >    哪一段停下等人。
  >
  > ⛔ **三条不许违反的**（与 `HANDOFF.md`「🔭 自动定时发布链路」同一份）：
  >
  > - **成功信号不许用"没报错"代替**（见 G6）；
  > - **`prepared` 不许自动升级成 `scheduled`**（见 G6c）；
  > - **`require_confirmation` 默认 true 不许顺手改掉**——
  >   想全自动是一行配置，但**那一行要用户自己按**，不是实现者顺手按。
  >
  > ⚠️ **别把 G9 排在 G6 前面。** 没有自动提交的"自动链路"，
  > 终点仍然是一个人工动作；先把线拉通只会让人以为它已经在自动跑了，
  > 而**"没跑"和"跑了但停在人工"在输出上一模一样**——
  > 这个项目已经为同型失效吃过两次亏（丢 263 篇合作帖、G1 空 dump）。
...
- [ ] **L0a** 装计划任务（就是 E3）　⏳ **G8、activate 与 assisted 配置之后由用户决定**

  > 当前故意不安装：G8 通过前不能激活发布边界，配置也仍为 `manual`。
  > 任务标识保持不变，daily/catchup action 已改为 `pipeline run`。
  - 用户操作，`MANUAL_STEPS.md` 第 9 步，一条命令
  - 【验收】`tools.schedule status` 显示三个任务已注册；次日流水线日志有新记录
- [x] **L0b** `pipeline status`：只读对账　✅ **2026-09-01 完成**
  ...
  - **先有"看得见"，再谈"自动跑"**
  - ✅ 第四轮修正：`待发布` 在激活前显示 `—`；激活后只计算边界之后的新帖，
    并按账号与 `source_refs` 求交集，只把最终 `scheduled` 扣除
  - ✅ `[pipeline]` 已完成双向配置审计：自治模式、日/月预算与死人开关都有
    实际消费者；`supervised` / `autonomous` 明确失败闭合
...
- [x] **L1a** `pipeline run`：每次从各阶段真相源重算差值并推进，不建立发布队列
- [x] **L1b** 预算闸（`[pipeline].monthly_budget_usd` / `daily_budget_usd`）。
      ⚠️ 按**真实 usage** 外推，不许按字符或张数猜（CR-40：低估一个数量级）
- [x] **L1c** `state/needs_human.jsonl` + 单页 HTML。
      **把"人"变成一个阶段，而不是一次中断**；沿用 `tools/layout.py::index`
      那条已被用户接受的路子（双击就能看）
- [x] **L1d** `autonomy = "assisted"` 代码路径：抓取/翻译/调图自动，发布停在待确认。
      当前配置故意保持 manual，G8 通过并 activate 后才切换

### L2 · 分流（G8 通过之后）

- [ ] **L2a** 七条分流规则（`PIPELINE_PLAN.md` 第 6 节）。本轮已完成用户指定的
      跨平台/金额/作者/素材/译文规则，其余 legacy 分流留后续。
      ⚠️ **不许为了"看起来更自动"去放宽它们**——它们和 C7 那七条一样，
      不是防回归，是防"跑得挺好就顺手优化掉"
- [x] **L2b** `[publish.trusted_owners]` 白名单。当前只信任目标账号自身；
      未知合作作者在任何付费请求前进入 needs_human
- [x] **L2c** 排期规则：Europe/Berlin **每天** 10:00 / 17:00，读取远端占用后
      分配最近空槽，并换算为 America/Los_Angeles UI 时间
...
- [x] **I1** 端到端串跑　—— **离线那半段已完成（2026-09-01）**

  - 顺序：回填 → 增量跑一次 → 翻译 → 生成审核清单 → 发布 1 条
  - 【验收】全流程无手工修数据，中间产物齐全
  - ✅ **`tests/tests_pipeline_e2e.py`**：在临时归档上把
    `delta → reconcile → 翻译 → 调图 → 离线硬闸 → ready_to_publish`
    **真的跑通**，并覆盖幂等复跑（零重复付费）与硬闸复查。
    ⚠️ **此前这一段一条断言都没有**：`tests_pipeline_assisted.py` 的夹具建在临时目录，
    而 `_run_unlocked` 里的 `compose_post(archive_root=cfg().archive_dir)` 读的是
    **真实 config 的归档**，于是那些用例全都停在 `offline_gate`，
    "翻译+调图做完之后会怎样"从来没被执行过。
  - ✅ 同一套还钉住了**四个跨进程 argv 契约**（`routes.delta` / `translate` /
    `localize_images` / `tools.publish_post`）。那四处是靠命令行字符串拼起来的，
    桩 runner 不校验参数名 —— 拼错了只会在 assisted 第一次真跑、
    或 approve 第一次开完浏览器之后才炸。
  - ⬜ **剩下的是真机那半段**：G8 发一篇 → activate → assisted → 装计划任务。
    顺序见 `docs/GO_LIVE.md`，状态用 `pipeline preflight` 查（别看手维护的表）。
- [ ] **I2** 更新 `README.md`
...
⚠️ **第四轮顺序覆盖旧依赖图**：生产证据 → G8 → activate → assisted →
用户决定是否安装计划任务。当前仍未安装，所以没有任何东西在后台自动运行。
...
**第四轮当前外部卡点只有新版 v2 生产证据与随后的 G8 真机验收**：

| 缺什么 | 挡住 | 补法 |
|---|---|---|
| 完整 v2 因果证据链与实测 UI 观察值 | 生产 G6/G6c | 用户手工提交未来测试排期，录全账号→提交→成功→Planner 就绪→同一卡片独立的时刻/正文/FB/IG/图片语义→final；开发侧只按 dump 回填并回查 |
| G8 真机结果 | `pipeline activate` 与 assisted | 证据回填后显式 `--submit`，手工取消，再复跑同项验证 journal 零浏览器跳过 |

G6/G6c 的单击提交、五态 journal、成功等待与回读代码已经完成；默认命令仍停在
提交前，只有显式 `--submit` 才开放且会强制严格 UI 约束。生产 `SIGNALS` 为空、
`ui_constraints_verified=false` 时会在接触浏览器前拒绝，这不是未实现，而是全局
红线 5 的证据门。

FB/IG 控件继续不读也不点，依赖 composer 默认全选；生产提交前证据化核对目标账号，
G6c 从独立 FB/IG 子语义回读，少任一渠道就转人工。
...
#### 同日再追加（CR-66）：停止没真停 → 问答处死锁；顺便把 20 个问题挪出出口

用户按 Enter 之后，观察项一路回车跳过时**卡住不动**，屏幕还在冒 `#18 click`。
根因：`stop_sweep` 只关了巡检，**监听器和 CDP 会话都还在**，问答期间每点一次
就 `create_task(record(...))` 排一个，而 `record()` 持锁、单条最坏十几秒；
`set_observations()` 抢同一把锁，排到队尾 —— 看起来就是死锁。

处置：按 Enter 后 `recording["on"]=False` + 逐个 `session.detach()`，
**真的关闸**；顺序是先 `sleep(0.9)` 保住最后一次去抖，再关闸，再问观察项。

顺带采纳用户的意见（「我根本不会去进行人工填写」——**这个抱怨是对的**）：
录交互和量 UI 边界是两件事、两个时间点，捆在一起只会逼人一路回车，
**填出来的是假数据，比空着更糟**。所以默认不问，新增
`--fill-notes <dump>` 事后补填（不连浏览器、只改 observations、
交互记录一条不动），`--notes` 保留就地问。
⚠️ **红线没松**：`_validated_probe_dump` 的必填校验一个字没动，变的只是何时问。

又一个真机 bug：观察项里**写中文会把整份 dump 写崩**
（`input()` 的 surrogateescape 留下孤立代理字符 → `write_text(utf-8)` 抛）。
新增 `console_text()` 还原 + `_write()` 兜底。

新增 16 条断言（`tests_publish` 118 → 134），**19 套 / 1241 项全绿**。

### 2026-09-01（收尾）· G1 首次录到真实数据；CR-67；交接文档同步

#### G1 状态从"零"变成"已录到、未收口"

用户当天跑通两次真实探查，`state/` 下有两份有价值的 dump：

| dump | 交互 | 覆盖 |
|---|---:|---|
| `..._195237_...` | 63 | 旧版 `www.facebook.com/professional_dashboard/*` |
| **`..._222850_...`** | **24** | **`business.facebook.com/latest/composer/`** |

那份 24 条覆盖了 **Create post → composer → Add photo/video → 正文
（`role=combobox`）→ 话题标签搜索/勾选（`searchbox`/`checkbox`）→ 表情 →
`Set date and time`（`role=switch`）→ 日期选择器 / `meridiem` / `minutes`
（`spinbutton`）**，24 张截图齐全、零截图失败。
**G3 / G4 / G5 要的定位证据基本都在里面了。**

⚠️ 但它 `finished_at=null` 且序号不连续（被 CR-66 的死锁中断），
**过不了 `_validated_probe_dump`**。
✅ **可以人工读来回填 `selectors.py`**（那本来就是 G1 产出的主要用途）；
⛔ **不许手工改序号"修好"它** —— 那两条校验就是用来证明记录没丢过的。

**所以 G1 的收口是两步**：用户重跑一次干净的探查（现在一次能走完）；
想开 `--strict` 时再 `--fill-notes` 补观察项。
**代码侧现在就能开工的是回填 `selectors.py`**（仍是 8 行 TODO）。

#### CR-67：一次失败的记录把序号永久空掉

整理交接文档时核对那份 dump 才发现的。`record()` 先 `self._counter += 1`
占号、再去截图，中途任何失败（抛异常/任务被取消/Ctrl+C）都会**永久空掉**
那个号，而严格校验要求 sequence 从 1 起连续 —— **内容完好的 dump 直接作废**。
改成由 `len(interactions)+1` 推出来：失败的那次不占号。
断言用"第 2 次截图必抛"的假页面验证：4 条事件仍得到 `[1,2,3,4]`。

#### 本轮交付

`main` = `d97b413`（+ 本次 CR-67 与文档），**19 套 / 1244 项全绿**。
`HANDOFF.md` 大改：新增「🎬 G1 现在到底走到哪了」与
「🧨 这一轮踩到的五个坑（CR-63~67）」，「接下来做什么」按新状态重写。
**计划任务仍未安装**（三个都没注册），按 E3 一贯做法没有替用户按。

### 2026-09-01（第二轮）· G 组 · G1 回填 `selectors.py`，G2–G5/G7 落地到「提交前一步」

#### 做了什么

按上一节留下的下一步（"人工读那份 24 条 dump，回填 `selectors.py`"）往下走，
并把这条链路补成一条**用户能真跑的命令**：

| 文件 | 变化 |
|---|---|
| `publish/selectors.py` | 8 行 TODO → **12 条 composer 定位 + 2 条旁证 + 8 条缺口登记**，全部带来源 |
| `publish/evidence.py` | **新增**：把每条定位回查到它自称的 dump |
| `publish/business_suite.py` | 五个 `ProbeRequired` 骨架 → G2/G3/G4/G5/G7 真实实现，G6 仍失败闭合 |
| `publish/journal.py` | **新增**：`state/published.jsonl` 的留痕与幂等（G6b） |
| `tools/publish_post.py` + `scripts/run_publish_post.bat` | **新增**：驱动入口，做到提交前一步 |
| `config.toml` `[publish]` | 新增 `ui_timezone`（留空=未实测，失败闭合）与 `ui_timeout_seconds` |
| `tests/tests_publish.py` | 152 → **196 项**；第 [5] 节整节重写 |

**19 套 / 1306 项全绿**（原 1244）；`compileall` 干净、`git diff --check` 干净、
`tomllib` 解析通过、新增 `.bat` 纯 ASCII + CRLF + 无 BOM。

#### 一处**故意反转**的测试断言，要说清楚

`tests_publish.py` 第 [5] 节原本断言的是
**"`publish/selectors.py` 里一个赋值语句都没有"**——那在 G1 之前是对的
（没有证据就不许有定位）。**但 G1 之后它会反过来挡住回填。**

换成了**更强**的守法：不是"不许有定位"，而是"**每一条都必须查得到出处**"。
`publish/evidence.py` 逐条回查 `role` + 可访问名 + 稳定属性，
本机 **14/14 全部命中真实 dump**。
⚠️ dump 在 `state/` 下、不进版本库，所以别的机器上是**跳过**而不是通过，
两种结果分开打印——**跳过不等于通过**。

**这是把红线 5 从"一句纪律"变成"一个可执行的检查"。** 编出来的定位会当场被打回。

#### 三处「dump 里没有，但不用重录也能做对」的处理

这三处如果按"缺证据就停"处理，整条链路会卡死在无关紧要的地方；
按"猜一个"处理，又踩红线 5。都改成**运行时发现 + 回读校验**：

| 缺口 | 处理 |
|---|---|
| composer 的 `<input type=file>` | 走 Playwright 的 **file chooser 通道**：点录到的 `Add photo/video`，截住浏览器弹出的选择器。**连选择器都不需要**，而且顺带当场回答了 G1 的第 2 问（弹得出=文件输入，弹不出=拖拽区，报错文案不同）|
| 小时那个 spinbutton | `Time input` 容器里三个 spinbutton，`minutes` / `meridiem` 有录到的名字，**剩下那个按排除法就是小时**；个数不是 3 就失败闭合 |
| 缩略图容器 | **不假装验过**：程序只确认"文件已交给控件"，把"数一遍缩略图"作为提示返回给人 |

#### 仍然失败闭合的三处（一个都没猜）

- **提交按钮 + 成功信号** → `submit()` 抛 `ProbeRequired`，逐条打出缺口。
  主流程停在提交前，最后那一下由人点。
- **FB/IG 渠道勾选** → `ensure_logged_in()` 在输出里点名"渠道由 composer
  打开时已选中的决定，提交前请人眼确认"。
- **UI 时区** → 新增 `[publish].ui_timezone`，**留空就失败闭合**并给出
  `--fill-notes` 的原话命令。⛔ 没有拿 `[publish].timezone` 顶上：
  那两个是不同的东西（"我们想按哪个时区排" vs "UI 认哪个时区"）。

#### 一条从 CR-64 直接继承下来的设计

**驱动前先证明这一页是活的。** `open_composer()` **自己开新标签页**
（Playwright 自己创建的 target 不会踩 CR-64），
`assert_page_usable()` 先跑一次 `page.evaluate("1+1")`，
超时就当场点名 CR-64 并让人按 F5——**绝不 `except: continue`**。
CR-64 的教训写在 CODE_REVIEW 18.2：把"设备没装上"变成"装好了但什么都没发生"，
两者在输出上一模一样。

#### 诚实的边界

**G2–G7 一项都没有勾选。** 除了 G1 的【记录】还缺，更重要的是：
**这些实现只在仿真 page 上跑过，真实 Business Suite 上一次都没跑过。**
CODE_REVIEW 18.9 那句话在这里原样成立——**mock 掉的边界就是没被测到的边界**，
而这一轮 mock 掉的正是整个浏览器。仿真能证明的只有逻辑：
时区换算（含两个夏令时切换日）、12 小时制换算、回读闸拦不拦得住、
排除法定位在个数变化时会不会失败闭合。**它证明不了选择器在真实 DOM 上能不能命中。**

#### 一处与并行线的交叉（记录，不是投诉）

写这一轮时工作区里出现了**另一条线正在改 `localize_images.py`**
（并把 `[image].timeout_seconds` 从 180 改成 720）。
按交接文件的协作纪律：**没有碰那个文件，也没有 `git add -A`**。
两边落在 `config.toml` 的不同段（`[image]` vs `[publish]`），
正是所有权表预期的形态，零冲突。

### 2026-09-01（第三轮）· G 组 · UI 时区定案：**跟发帖者设备的本机时间走**

#### 用户实测推翻了一条写进任务书的假设

`PUBLISH_PLAN` 第 3.3 节原话是「Business Suite 显示的时间**跟 Page 的时区设置走**」。
**实测不是。** 用户 2026-09-01 确认：**UI 上那个时刻跟发帖者设备的本机时间走。**

按项目约定「计划与现实冲突时，以现实为准并记录」——3.3 节保留原文并加了定案段。

#### 落地

- `[publish].ui_timezone = "America/Los_Angeles"`（这台机器的时区，
  Windows 显示 `(UTC-08:00) Pacific Time (US & Canada)`）；
- `[publish].timezone = "Europe/Berlin"` 语义不变（想让德国受众几点看到）；
- `business_suite.assert_ui_timezone_is_device()`：每次排期前核对
  **配置时区与本机在「目标时刻」的偏移一致**，对不上失败闭合。
  换机器、或有人改了 Windows 时区，会当场被抓住而不是静默排错时刻；
- `set_schedule()` 的回读改成**两个时刻一起给**：
  `UI 显示 09/08/2026 1 : 00 AM（America/Los_Angeles）＝ 目标 2026-09-08T10:00+02:00`。
  只给一个的话，人要么核对不了屏幕、要么以为排错了。

#### 一个不写下来一定会被踩的坑：**两地夏令时切换日不是同一天**
...
于是一年有**两段约一周的窗口**，时差从 9 小时变成 **8 小时**
（2026-10-25~11-01、2027-03-14~03-28）。

⛔ **任何"减 9 小时"的手算、或写死偏移，都会在那两段里把帖子排错一小时。**
代码走的是绝对时刻换算（`astimezone`），四种情况各有一条断言：
常规 9 小时、柏林已回冬令时而本机还在夏令时（8 小时）、
两地都回冬令时（9 小时）、本机已进夏令时而柏林还没（8 小时）。

⚠️ 核对用的是**目标时刻**的偏移，不是"今天"的偏移——
拿今天去判，会在上面那两段窗口里把**正确**的配置判成错的。有断言钉住这一点。

#### 顺带：K→G 的交接在真实归档上第一次成立

K 组当天真跑了一张（`3975547640610092585` 的 `01.jpg`，`images_de.jsonl` 有记录）。
`tools/compose_publish.py` 在真实归档上跑出来：**那一篇的图源已经是 `media_de`（德语图）**，
另外两篇仍按设计逐张告警回退原图。
**这是 K 与 G 的所有权契约第一次在真实数据上双向成立**，此前只有夹具验证过。

### 2026-09-01（第四轮）· 用户两条确认消掉两个卡点；把"还差什么"写成任务书

#### 用户确认的两件事

| 确认 | 影响 |
|---|---|
| **那张德语图「符合要求、完全可用」** | K9 的【验收】在**这一张**上过了；⛔ 但范围是"最新 3 篇的全部配图"，11 张只跑了 1 张，**仍不勾选**。勾选条件收敛成"补跑剩下 10 张并逐图确认" |
| **进 composer 时 FB 与 IG 两个渠道默认全勾选** | `GAPS.composer_placement_toggles` **降级**：不再挡住「同时发」这条范围 |

#### 渠道那条缺口是**降级，不是关闭**——这个区别必须写清楚

原来写的是「『FB + IG 同时发』这条用户点名的范围做不了」，**那是高估了**：
默认全勾选，本来就不需要点。

**但仍然缺一样：程序读不出当前勾了哪几个。** 所以：

- `ensure_logged_in()` 现在如实说「渠道默认全勾选，程序不去改它；
  但程序也**读不出**当前勾了哪几个，提交前请人眼扫一下」；
- ⛔ **没有改成"假装核对过"**。只有要**改**选择（只发一边）
  或想让程序自己回读校验时，才需要补录。

> 这一条是「把 A（不用点）说成 B（已经核对过）」的典型诱惑。
> 项目里同型的教训已经有两条：`"code" in d` 判键不判值（CR-19，差 478 条脏数据）、
> `except: continue` 把"没装上"说成"装好了但没事发生"（CR-64）。

#### 把"自动定时发布链路还差什么"写成了任务书

分散在几处的"以后要做"收拢成三个有判据的任务项：

| 任务项 | 内容 |
|---|---|
| **G6**（补充实施顺序） | 重录探查 → 回填提交按钮 → `submit()` 实现成「点 → **等成功信号** → 返回标识」。⛔ 顺序不许颠倒：先写点击、想着"成功信号以后再补"，等于先造出一个会真的往外发帖、却判断不了发没发出去的东西 |
| **G6c**（新增） | 提交后回排期列表**再看见这一篇**才写 `scheduled`。G6 的成功信号回答"这次提交成了没有"，G6c 回答"它现在确实排在那儿"，两者不能互相代替 |
| **G9**（补充任务书） | 真实工作量不是"实现流水线"，而是三件小事：一个只负责编排的入口（不许自己干活）、待确认清单落盘、`[pipeline].autonomy` 真的被读（现在是死旋钮） |

同一份摘要写进 `HANDOFF.md` 的「🔭 自动定时发布链路：还差什么」与
`PUBLISH_PLAN.md` 第 8 节，三处一致。

⚠️ **三处都写了同一条排序警告**：**别把 G9（串链路）排在 G6（自动提交）前面。**
没有自动提交的"自动链路"终点仍然是一个人工动作，先把线拉通只会让人以为
它已经在自动跑了 —— 而**「没跑」和「跑了但停在人工」在输出上一模一样**。

#### 顺带修掉一句被自己改法弄过时的话

`compose.py` 里 IG 约束缺失时的告警原话是
「当前只完成离线组装，**禁止进入浏览器发布**」——写它的时候确实没有浏览器路径。
现在有了（`tools/publish_post.py`），非严格模式**会**带着这条告警继续走，
于是它变成一句**程序自己都不遵守的禁令，比没有更糟**。
改成如实说：没实测过、composer 可能自己拒、要挡住就开 `--strict`。
定时窗口那条的禁令**仍然成立且没有松动**，只把"禁止"的对象说准：
禁止的是**采信那两个 API 占位数**，不是禁止发布。

### 2026-09-01（第五轮）· K 组 · 第二张文字图实测与精确媒体安全闸

- 按用户“无文字图不必生成”的新范围，人工检查较新帖子
  `3973012230169803390` 的配图，选中 `media_index=0`：标题、长评语、品牌/型号、
  人名和 backer 编号同时存在，适合作为第二次实验；其余图片未调用。
- high 真实 edits 成功：1080×1080 → 1088×1088，149.125 秒，dHash=6，
  usage input/output/total=2673/7336/10009；按配置上游费率为 US$0.2369。
  输出 JPEG 可完整解码，原图与 manifest 哈希未变，输出哈希与 `images_de.jsonl`
  一致，`model=gpt-image-2 / model_verification=catalog`。
- `review.md` 已生成原图/德语图并排页；第一张实验已获用户确认，第二张等待用户人工确认。
  两张当前版本样本的上游中位成本是 US$0.2458/张，dHash 分布为 6、9；样本仍太少，
  `dhash_max_distance=-1` 保持不变。
- 第二张成功后暴露出旧 CLI 的危险语义：`--limit 1` 限制的是“剩余队列”，不是固定媒体。
  首图当前时会前进到下一张。一次旧命令复跑因此开始等待 `media 1`，约 30 秒后中断；
  本地无 `02.jpg`、无对应 JSONL，但网关是否已经处理/计费未知。发现后没有再发付费请求。
- 新增 `--media-index N`（0 对应 `01.jpg`），要求恰好一个 `--post-id`，并让
  dry-run、estimate 与正式入口共享同一过滤。双图夹具证明：精确复跑当前首图时 0 API、
  0 写盘，即便次图仍待处理也不会滑过去；不存在/非图片序号在联网前失败。
- 图片专项、翻译审校交接测试全绿；真实归档精确 dry-run 为“当前跳过 1 / 待处理 0”，
  精确 estimate 全程离线并正确读到两张样本。其余 9 张不再作为必须补齐的批次；
  后续只对人工确认含文字且业务需要的图片逐张授权、逐张处理。
