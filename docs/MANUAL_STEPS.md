...
逐账号给出「归档 / 可发 / 待译 / 待调图 / 待发布」。**待译和待调图就是这一轮会花钱的条数。**

```powershell
scripts\run_translate.bat --estimate
```

按当前待译正文估 token 与费用区间。⚠️ 它**不含 thinking 的 reasoning 输出**，是下界不是上界。
...
历史估算：出图约 US$0.211/张、约 2.5 分钟/张（2026-09-11 样本）。**这是旧数字，不是当前保证**；
真实跑完之后用当前账单回填，在那之前不要把它写成结论。
...
`--process` 必须与 `--run` 一起给。`--once` 只跑当前到期的一轮，先用它，别一上来就常驻。
