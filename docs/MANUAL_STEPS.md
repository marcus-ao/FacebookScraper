...
| 第 5 步 | 抽查媒体 | ✅ 已代你验收（见下） |
| **第 6–7 步** | **配翻译 API、试跑与审校** | ⬅️ **现在轮到你，且只差这个** |
| 每日增量方案 | 登录态 + CDP（方案 B） | ✅ **你已定案**，文档已同步，见下 |
...
> `config.toml` 里 `runs_per_day` 和降频开关都可以调，**调低就是直接降低风险**。
> 顺带：你们 IG 最近一个多月没发新帖，天天查其实没什么收益。
...
FacebookScraper\
  ...
  archive\<平台前缀>_<账号>\
    ...
    _capture_*.json       原始响应。**别删** —— 解析器再出问题时靠它离线重建，
                          不用你重滚
  state\
