...
| 变量 | 是什么 |
|---|---|
| `FEISHU_WEBHOOK_OPS` | 业务群机器人地址。收待审、积压、排期回执、晨间摘要 |
| `FEISHU_WEBHOOK_TECH` | 技术群机器人地址。收系统告警与监测/落档播报 |
...
0. 恢复 9224 探测会话后跑一轮真实探测：技术群应收到两张卡（先「监测到新帖」再「原帖抓取完成」），逐篇明细与 `archive/` 里实际落档的目录、图片数一致；故意停掉探测 Chrome，技术群应收到系统告警——这一条验的是「沉默会说话」；
1. 业务群接待审/积压/排期成功/失败，技术群接系统告警与播报，两个群不串；
