...
⚠️ **`SIZE_STEP`、像素上下限、最大边长和 3:1 这组尺寸契约是按 gpt-image-2 标定的，2.5 没有实测。** 自检请求 816×816 并核对返回尺寸，契约变了会当场露出来；自检不过就不要切。

### 5.2 标定「图内文字到底改没改」的告警线

每张德语图都会记录 `changed_pixel_ratio`（改动像素占比）。占比恰好为 0 时审校台会显著提示——它有两种成因，本地分不开：图里本来就没有要译的英文（原样返回是对的），或者模型没照做（这次钱白花了）。**人对照左右两张一眼能分开，所以这里不拦，只提示。**
...
业务在审校台图片页可以直接上传替换某一张（JPEG / PNG / WebP）。替换之后：

- 这一张按**人工图优先**使用，程序不会再重生成覆盖它，该张的模型优化入口也会停用并写明原因；
- 这篇**状态不变**，继续留在系统里走排期发布——上传是换素材，不是转交人工；
- 被替换掉的旧图移到该帖 `media_de/superseded/`，付费产出不销毁，反悔时能取回；
