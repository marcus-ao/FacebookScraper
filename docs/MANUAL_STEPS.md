...
这些预检结果不再挡住审校台的单篇排期。内容合格就可以冻结并选择时刻。提交前要在 `[publish]` 写上从目标页 URL 取得的 `asset_id` 和 `business_id`，并确认发布 Chrome 已登录。缺这两个值时可以冻结和选时刻，不能提交。首次测试不要求另一渠道已经成功，也不要求流水线已经激活。G8 和远端图片核验仍按原标准另验。

**更新到发布精简版本时，先核对这两个新增配置。** 它们在 `config.toml` 中默认为空；填入与当前目标账号对应的数字 ID，不从旧录证自动迁移，也不要放进只允许路径绑定的 `config.local.toml`。人工撤销核验同样需要这组配置；普通月历刷新继续使用原录证入口。按 [§13](#13-更新并启动审校台) 的启动脚本构建并使用同版本前端。旧任务和快照按原指纹版本读取，不批量重写账本；受管实例与回退的版本约束见 [§5.3](#53-业务自己换图)。
...
操作前确认 §16.1 的当前资产配置齐全；目标时刻回读或日历刷新成功都不能替代这里的整月核实。
