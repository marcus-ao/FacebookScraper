...
> **建立日期：2026-08-31。**
>
> **状态（2026-09-01 第四轮更新）**：G0–G7 的代码已完成，含 probe v2、
> `SubmitResult` / `ScheduledReadback` / `PublishAttempt`、单次提交、五态追加式 journal
> 与内容日历回读。单帖命令默认仍停在提交前，只有显式 `--submit` 才进入 G6。
> **生产证据仍故意失败闭合**：现有旧 dump 不能按因果顺序证明目标账号上下文、
> 提交动作、成功语义、Planner 数据就绪与同一卡片内独立的时刻/正文/FB/IG/图片语义，
> 所以证据登记表不猜、不填，`--submit` 会在碰浏览器前拒绝。用户需用新版 probe
> 手工提交一次未来测试排期并保持最终页面不动，再把完整 v2 dump 交回；验证后人工取消。
> `--submit` 会强制使用该 dump 中经复核的严格 UI 约束，配置占位值不能放行真实提交。
> 状态机在 click 前先 fsync 一个 `submit_ambiguous` intent，关闭 click 后进程崩溃却只剩
> `prepared` 的重复发布窗口；提交前先读同条件 Planner 基线，提交后两端 remote ID
> 若都存在必须相等。批量槽位只允许落在运行时解析并由 v2 证据支持的可见日期范围内。
>
> ⚠️ G8 仍未执行，因而流水线没有激活，也不会处理历史帖。FB/IG 渠道不点击，
> 依赖 composer 默认全选；G6c 若明确回读到缺少某一渠道则转人工，绝不记 scheduled。
> Business Suite UI 使用本机 `America/Los_Angeles`，业务槽位固定为
> `Europe/Berlin` 每天 10:00 / 17:00，代码负责两地夏令时换算。
>
...
v2 工具对 click / input（700ms 去抖）/ change / submit 逐条原子刷新 JSON，
点击命中内层图标或 `<span>` 时还会记录语义祖先；同时被动记录去敏 URL 变化、
toast/alert/status、最终页面语义与遮罩截图。可编辑节点在浏览器端和 Python 端双重
去敏，不记录 class/CSS path、cookie、输入值或凭据。交互与快照共享连续的
`evidence_order` 和 `page_id`，用于证明 account → submit → success → Planner → final
发生在同一页面和正确顺序。观察值可在录制时用 `--notes` 补记，或事后用
`--fill-notes` 原样补录；不知道的项留空，绝不猜。

> ~~**当前状态：工具与离线 recorder 测试已完成，但用户尚未实际跑 G1。**~~
>
> **2026-09-01 更新：用户已跑，`selectors.py` 已回填。**
>
> 两份 dump（`state/`，不进版本库）：
>
> | dump | 交互 | 界面 | 用途 |
> |---|---:|---|---|
> | `publish_probe_20260831_222850_347719.json` | **24** | **`business.facebook.com/latest/composer/`** | **回填就靠它** |
> | `publish_probe_20260831_195237_008047.json` | 63 | `www.facebook.com/professional_dashboard/` | **另一个界面**，只作旁证 |
>
> ⛔ **两份不许混用。** 旧版后台那份是 FB 单平台的创作者后台，没有 IG 那一路；
> 它唯一被采信的用途是**证明 Meta 的 Photo/video 按钮背后确实是一个真的
> `<input type=file multiple>`**（第 33/34 条），G3 因此可以放心走文件选择器通道。
>
> **回填出来的 12 条定位**（每条都带「对应哪一步 / 出自第几条交互 /
> 什么信号说明它失效」）：`create_post_button`、`composer_done_button`、
> `add_media_button`、`caption_box`、`hashtag_menu_button`、`hashtag_dialog`、
> `hashtag_search_box`、`schedule_switch`、`schedule_date_input`、
> `schedule_time_group`、`schedule_minutes`、`schedule_meridiem`。
>
> **机器校验**：`publish/evidence.py` + `tests_publish.py` 第 [5] 节把每一条
> 回查到它自称的 dump（role + 可访问名 + 稳定属性都要对上），本机 14/14 命中。
> **红线 5 从此是一条可执行的检查。** dump 不在时是"跳过"不是"通过"，分开打印。
>
> **旧 dump 没录到的写在 `selectors.GAPS`。** 当前生产缺口是同一份完整 v2 dump：
> 目标 FB/IG 账号上下文、可信提交交互、被动成功语义、Planner 数据就绪信号、
> 结构化排期卡片与完整 final；排期窗口和 IG 上限也必须在观察项中实测复核。
> FB/IG 勾选控件仍不录不点，因为进入 composer 时默认全选；它由提交后独立渠道
> 子语义回读兜底。
...
| 4 | ~~**FB 与 IG 两个渠道的勾选控件在哪**~~ | ✅ **2026-09-01 用户实测消解**：进 composer 时**两个渠道默认全勾选**，不用点。只有要**改**选择（只发一边）或想让程序自己回读校验时才需要补录 |
| 5 | 定时开关的位置 | — |
| 6 | **日期/时间选择器：能不能直接输入，还是必须点选；以及 UI 显示的是哪个时区** | ⚠️ **这是整组最容易出错的一步**，见 3.3 |
| 7 | 目标账号上下文、提交按钮、提交成功信号，以及 Planner 数据就绪与同一卡片内独立的时刻/正文/FB/IG/图片语义 | 没有完整、同页、有序的证据链就无法防止发错账号，也无法区分成功、加载中、空日历或正文中的假阳性文本 |
...
> ✅ **2026-09-01 用户实测定案：UI 上那个时刻跟发帖者设备的本机时间走。**
> 不是 Page 的时区设置，也不是受众所在时区。
> 本节下面那句"跟 Page 的时区设置走"是**定案前的假设**，已被推翻，保留只为存档。
>
> 落地形态：
>
> - `[publish].ui_timezone = "America/Los_Angeles"`（= 这台机器的时区）；
> - `[publish].timezone = "Europe/Berlin"` 仍然是「想让德国受众几点看到」；
> - 代码把目标时刻**显式换算**到 `ui_timezone`，并在每次排期前核对
>   **配置时区与本机在目标时刻的偏移一致**（`assert_ui_timezone_is_device`），
>   对不上失败闭合——换机器或改了 Windows 时区就会当场被抓住。
>
> ⚠️ **两地夏令时切换日不是同一天**（2026：柏林 03-29 / 10-25，
> 本机 03-08 / 11-01），所以一年有两段约一周的窗口时差从 **9 小时变 8 小时**
> （2026-10-25~11-01、2027-03-14~03-28）。
> ⛔ **任何形式的"减 9 小时"手算或写死偏移都会在那两段里错。**
> 四种情况都有测试钉着。
...
## 4. G2–G7 · 实现（**代码已落地；生产提交等 v2 证据**）

~~G1 之前，这几项只写**函数签名 + 契约 + 测试骨架**，不写选择器。~~

**现在的形态**：默认准备命令与显式提交命令共用前半段，只有后者进入不可回滚边界：

```
compose.py 离线硬闸 → 人点头 → 附着 9223 → 新开标签页进 composer
  → G2 证据化账号核对 → G3 交图 → G4 文案+回读 → G5 排期+回读
  → 默认停下
  → [仅 --submit] 单击一次 → 成功语义 → Planner 结构化回读 → scheduled
```

入口：`scripts\run_publish_post.bat --post-id <id> --at <ISO>` 默认停在提交前；
只有显式增加 `--submit` 才尝试自动提交并回读。`--submit` 同时强制严格 UI 约束，
当前生产证据登记为空，因此会在碰浏览器前失败闭合；这不是把缺证据当作已验收。

⚠️ **下面每一项的【验收】都没勾**：实现只在仿真 page 上验证过，
**真实 Business Suite 上一次都没跑过**。
...
2b. **话题标签逐个原样照搬**——同样直接复用 `translate.py::hashtags_preserved`
   （CR-61）。标签与金额在 `translate.py` 里本来就是**同一类**「不可改内容规则」，
   `run_translate` 把两者的 violations 合成一个列表、任一不过都不写盘；
   但本节原先只列了金额，于是发布侧也只再判金额。
   ⚠️ **写盘侧判过不等于发布侧不用判**：`translated.jsonl` 是译文真相源，
   人工审校的修正就是直接改它（`review.md` 里写着"直接修改本文件不会回写数据"），
   而手工改动**不经过** `run_translate` 的写盘闸。改错标签 =
   德语帖挂错话题或漏掉品牌标签，IG 那侧标签还是触达路径，不是装饰。
   与第 4 条的**标签数量**上限不是一回事：那道闸问"会不会被 UI 拒绝"，
   这道闸问"还是不是原帖那几个标签"；
...
> ✅ **验收通过：2026-08-31（同步审查轮）。** 三篇全部组装成功、零拦下：
>
> ```
> facebook  / 122123185335379375   5 图
> instagram / 3973012230169803390  5 图（合作帖，输出点名原作者 Neakasa Global）
> instagram / 3975547640610092585  1 图
> ```
>
> 卡了很久的那一半其实在 F 侧（CR-47）：`translate.py` 的待译队列是**最老优先**、
> 只有 `--limit`，没有按 post_id 选帖的能力，所以这三篇一直补不上译文。
> 用户授权后加了 `--post-id`/`--latest-posts`，三篇已真实翻译。
>
> **现在有命令可以复跑这条验收了**（此前 `compose_post()` 的唯一调用方是测试文件，
> 这条【验收】没有任何用户可执行的路径 —— CR-58）：
>
> ```bash
...
> ```
>
> 零浏览器、零网络、零写盘。`--strict` 在 G1 未完成时正确失败闭合。
>
> ⚠️ **顺带发现一个原先没人知道的 G5 阻塞（CR-60）**：
> `ZoneInfo("Europe/Berlin")` 在这台 Windows 上**直接抛 `ZoneInfoNotFoundError`**
> （`zoneinfo.TZPATH` 为空，Windows 不自带 IANA 时区数据库）。
> 第 3.3 节要求的显式时区转换与夏令时切换日测试，缺 tzdata 时**根本无法实现**。
> 已在 `requirements.txt` 加 `tzdata`（纯数据包）并验证 2026 年两个切换日的偏移。
> **不要改成写死 UTC 偏移绕过去** —— 那会在切换日发错时刻，且没人会立刻发现。
>
> ⚠️ **`media_de/` 同序号多候选的语义已改（CR-48，用户拍板）**：
> 原先见到多个候选就硬失败，但 K 组**刻意支持**「程序图与人工修订版并存、人工优先」——
> 于是 K 专门为设计同事设计的场景会让这篇发不出去。
> 现在读 `images_de.jsonl` 判程序所有权，非程序产出的那个优先并显式告警。
> 仍然失败闭合的只有"多个都不是程序产出"（真的分不清该发哪张）。
>
> **原实现状态（2026-08-31 上午，保留作历史）：** `publish/compose.py` 已完成当前译文与
> 源正文指纹、`translate.money_preserved`、Pillow 完整像素解码/非零字节、
> 缺失完整性标记、残缺轮播、混合/脏媒体项全量失败闭合、
> `media_de` 逐图优先/原图逐图告警回退、合作帖原作者提示、显式时区 datetime 等硬闸。
> IG 四类限制与定时窗口必须绑定 config 人工确认的同一份真实 probe：严格模式会解析
> dump，核对 9223/profile/必填观察/可信交互/截图，并把注入数值逐项与记录比对；
> G1 前必然失败闭合，绝不消费 `10 分钟/75 天` 的 API 占位值。
>
...
```
routes/delta.py 抓到新帖        ✅ 通（计划任务仍未安装 = L0a）
   → translate.py            ✅ 通    德语译文
   → localize_images.py      ✅ 通    德语图（K 组，已真跑一张并经用户确认可用）
   → publish/compose.py      ✅ 通    离线硬闸
   → tools/publish_post.py   ✅ 通    默认停在提交前；--submit 显式启用
   → 【提交 + 成功信号】      ✅ 代码完成；生产证据登记仍为空
   → Planner 回读 scheduled   ✅ 代码完成；真实验收归 G8
```

> **2026-09-01：每一段自己都通了，缺的是最后那一下和把各段串起来的那根线。**
> 剩余任务已写成任务项：`IMPLEMENTATION_PLAN.md` 的 **G6 / G6c / G9**，
> 摘要见 `HANDOFF.md`「🔭 自动定时发布链路：还差什么」。
>
> ⚠️ **别把 G9（串链路）排在 G6（自动提交）前面。**
> 没有自动提交的"自动链路"终点仍然是一个人工动作，
> 先把线拉通只会让人以为它已经在自动跑了——
...
compose.py 硬闸          ✅ 已完成
        ↓
G1 探查                  🟡 已录到 dump、已回填定位；观察项与提交那一段没走完
        ↓
G2 → G3 → G4 → G5 → G7   ✅ 已实现（只在仿真 page 上验证过）
        ↓
G6/G6c 提交与回读        ✅ 代码完成；生产证据登记为空时在浏览器前失败闭合
        ↓
G8 端到端（需要 F 的译文 + K 的德语图）
```

**2026-09-01（第四轮）：代码侧卡点已收口，剩余是完整真实证据与 G8。**

| 缺什么 | 只挡住 | 状态 |
|---|---|---|
| ~~`[publish].ui_timezone`~~ | ~~真实跑一次准备~~ | ✅ **已定案**：用户实测确认**跟发帖者设备的本机时间走**，已填 `America/Los_Angeles` 并加了设备一致性核对 |
| ~~FB/IG 渠道勾选控件~~ | ~~「同时发」这条范围~~ | ✅ **已消解**：用户确认进 composer 时**两个渠道默认全勾选**，本来就不用点。只剩「程序读不出勾了哪几个」，降级为提交前人眼扫一下 |
| **完整 v2 证据链 + 实测 UI 观察值** | **G6/G6c 生产解锁** | ⛔ **仍缺**。需同页、有序证明账号、提交、成功、Planner 就绪、结构化卡片和 final；`--submit` 强制严格约束 |

**所以现在的形态是**：G2–G7 状态机与安全闸都已实现；默认命令仍由人完成最后一下，
而显式 `--submit` 只会在证据和严格限制全部复核后自动单击一次并回读。
整条自动链路的剩余任务见 `IMPLEMENTATION_PLAN.md` 的 **G6 / G6c / G9**
与 `HANDOFF.md` 的「🔭 自动定时发布链路：还差什么」。
...
**2026-09-01：G1 录到了，这条规矩换了形态但没有放松。**
现在的规矩是：**每一条定位都必须能回查到它自称的 dump。**
`publish/evidence.py` 逐条比对 `role` + 可访问名 + 稳定属性，
`tests_publish.py` 第 [5] 节把它变成一条断言——**编出来的定位会当场被打回**。

原来那句话的**理由**一个字没变：提交一个"看起来合理"的选择器比不提交更糟，
下一个人会以为它被验证过。区别只是：过去用"一个都不许有"来保证，
现在用"每一个都查得到出处"来保证，而后者还允许项目往前走。

⚠️ dump 在 `state/` 下、**不进版本库**，所以别的机器上回查会**跳过**。
**跳过不等于通过**，两种结果必须分开打印（已经是这么做的）。
