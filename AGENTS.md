...
⛔ **已登记的例外：2026-09-19 为 2026-09-20 试运营，用户决定把 [REQUIREMENTS §10.1–§10.4](docs/REQUIREMENTS.md#10-五阶段验收状态) 标记为 `真实通过*`，标记时没有可复核的真实证据。** 这条例外只覆盖那四节里带星号的行，判据与限制写在 [REQUIREMENTS §0](docs/REQUIREMENTS.md#0-状态词)。它**不放宽上面三条规则**：新的结论仍按原判据写，带星号的行不能当验收依据、不能解除闸门；取得真实证据后去掉星号并补上日期、输入和证据位置。不要把这次例外当成以后可以跳过证据的先例。
